--ZetaEditor.Lua
local this={
	modName = "Zeta Editor",
	modDesc = "Allows you to retreive and edit lua tables in-game.",
	modCategory = "Extensions",
	modAuthor = "(ZIP)",

    --Labels
    receiverSetBaseLabels = {
        "Fire rate",
        "Aim-Assist Distance",
        "Draw Speed",
        "Unknown4",
        "Unknown5",
        "Ironsight 1",
        "Ironsight 2",
        "Reload Speed",
    },
    receiverWobblingLabels = {
       "Unknown1",
       "Unknown2",
       "Unknown3",
       "Unknown4",
       "Unknown5",
       "Unknown6",
       "Unknown7",
    },
    receiverSystemLabels = {
        "Equip Type",
        "Reticle Type",
        "Fire Mode",
        "Unknown4",
        "Unknown5",
        "Unknown6",
        "Unknown7",
        "Unknown8",
        "Unknown9",
        "Unknown10",
        "Unknown11",
        "Unknown12",
    },
    damageLabels = {
        "Attack ID",
        "Lethal Damage (UI)",
        "Unknown3",
        "Unknown4",
        "Unknown5",
        "Unknown6",
        "Unknown7",
        "Unknown8",
        "Injury Type",
        "Injury Part",
        "Unknown11",
        "Unknown12",
        "Physically Simulated",
        "Unknown14",
        "Knockback",
        "Tranquilizer",
        "Stuns",
        "Unknown18",
        "Unknown19",
        "Unknown20",
        "Fire",
        "Unknown22",
        "Gas",
        "Unknown24",
        "Unknown25",
        "Electric",
        "Unknown27",
        "Unknown28",
        "Damage Source",
        "Lethal Damage Value",
        "Non-lethal Damage Value",
        "Impact Force",
    },
    gunBasicLabels = {
        "Weapon ID",
        "Receiver ID",
        "Barrel ID",
        "Ammo ID",
        "Stock ID",
        "Muzzle ID",
        "Muzzle Option ID",
        "Sight ID",
        "Sight 2 ID",
        "Underbarrel ID",
        "Laser/Flashlight ID",
        "Laser/Flashlight 2 ID",
        "Weapon Grade",
    },
    receiverLabels = {
        "Receiver ID",
        "Attack ID",
        "Set ID",
        "Wobbling ID",
        "System ID",
        "Sound ID",
    },
    barrelLabels ={
        "Barrel ID",
        "Barrel Param Sets Base ID",
        "Barrel Length ID",
        "Has Scope Mount",
        "Unknown1",
        "Has Side Mount",
        "Has Under Mount",
    },
    barrelSetBaseParamLabels = {
        "Unknown1",
        "Unknown2",
        "Unknown3",
        "Unknown4",
        "Unknown5",
        "Unknown6",
        "Unknown7",
    },
    magazineLabels = {
        "Ammo ID",
        "Equip Ammo ID",
        "Magazine Capacity",
        "Total Carry Capacity",
        "Bullet ID",
    },
    muzzleOpLabels = {
        "Muzzle Option ID", 
        "Grouping Bonus",
        "Supressor Durability",
        "Is Suppressor",
    },
    sightScopeLabels = {
        "Scope ID",
        "Zoom Distance 1",
        "Zoom Distance 2",
        "Zoom Distance 3",
        "Scope UI ID",
        "Is Booster Scope",
        "Is Nightvision",
        "Is Built-in Scope", 
        "Is Range Finder",
        "Is Range Finder w/ Bullet Drop Indicator",
    },
    stockLabels = {
       "Stock ID",
       "Unknown1",
       "Unknown2",
    },
    underBarrelLabels = {
        "Underbarrel ID",
        "Receiver ID",
        "Ammo ID",
        "Unknown1",
    },
    bulletSetBaseLabels = {
        "Unknown1",
        "Unknown2",
        "Unknown3",
        "Unknown4",
        "Unknown5",
        "Unknown6",
        "Unknown7",
        "Unknown8",
        "Unknown9",
        "Penetrate Level ID 1",
        "Penetrate Level ID 2",
        "Unknown12",
    },
    bulletLabels = {
        "Bullet ID",
        "Initial Force",
        "Drop-off Force",
        "Unknown4",
        "Param Set Base 1",
        "Param Set Base 2",
        "Unknown7",
        "Unknown8",
        "Ricochet Size",
        "Bullet Type",
        "Bullet Ammo",
        "Unknown12",
        "Equip Type",
    },
    --IDs
    atkIDs = {
        ATK_10001 = 319,
        ATK_10004 = 320,
        ATK_10006 = 321,
        ATK_10015 = 322,
        ATK_10024 = 323,
        ATK_10035 = 324,
        ATK_1003a = 466,
        ATK_10101 = 325,
        ATK_10102 = 326,
        ATK_10105 = 327,
        ATK_10107 = 328,
        ATK_10117 = 329,
        ATK_10125 = 330,
        ATK_10134 = 331,
        ATK_10136 = 332,
        ATK_10201 = 333,
        ATK_10205 = 334,
        ATK_10214 = 335,
        ATK_10216 = 336,
        ATK_10302 = 337,
        ATK_10304 = 338,
        ATK_10306 = 339,
        ATK_10307 = 340,
        ATK_10403 = 341,
        ATK_10405 = 342,
        ATK_10407 = 343,
        ATK_10503 = 344,
        ATK_10515 = 345,
        ATK_10526 = 346,
        ATK_10603 = 347,
        ATK_10615 = 348,
        ATK_10626 = 349,
        ATK_10626_Grazed = 350,
        ATK_10637 = 351,
        ATK_10702 = 352,
        ATK_10704 = 353,
        ATK_10706 = 354,
        ATK_20002 = 355,
        ATK_20006 = 356,
        ATK_20015 = 357,
        ATK_20103 = 358,
        ATK_20116 = 359,
        ATK_20119 = 467,
        ATK_20203 = 360,
        ATK_20215 = 361,
        ATK_20225 = 362,
        ATK_20302 = 363,
        ATK_20307 = 454,
        ATK_20309 = 468,
        ATK_30001 = 364,
        ATK_30014 = 365,
        ATK_30102 = 366,
        ATK_30117 = 455,
        ATK_30119 = 469,
        ATK_30201 = 367,
        ATK_30232 = 368,
        ATK_30237 = 456,
        ATK_30239 = 470,
        ATK_30303 = 369,
        ATK_30325 = 370,
        ATK_40001 = 371,
        ATK_40004 = 372,
        ATK_40012 = 373,
        ATK_40015 = 374,
        ATK_40023 = 375,
        ATK_40032 = 376,
        ATK_40035 = 377,
        ATK_40042 = 378,
        ATK_40045 = 379,
        ATK_40102 = 380,
        ATK_40105 = 381,
        ATK_40115 = 382,
        ATK_40123 = 445,
        ATK_40126 = 383,
        ATK_40203 = 384,
        ATK_40207 = 385,
        ATK_40304 = 386,
        ATK_40306 = 387,
        ATK_50002 = 388,
        ATK_50015 = 389,
        ATK_50026 = 390,
        ATK_50026_Grazed = 391,
        ATK_50033 = 393,
        ATK_50035 = 394,
        ATK_50047 = 392,
        ATK_50102 = 395,
        ATK_50115 = 396,
        ATK_50126 = 397,
        ATK_50126_Grazed = 398,
        ATK_50133 = 400,
        ATK_50136 = 401,
        ATK_50147 = 399,
        ATK_50202 = 402,
        ATK_50215 = 403,
        ATK_50226 = 404,
        ATK_50226_Grazed = 405,
        ATK_50237 = 406,
        ATK_50303 = 407,
        ATK_60001 = 408,
        ATK_60002 = 409,
        ATK_60007 = 410,
        ATK_60013 = 411,
        ATK_60102 = 412,
        ATK_60103 = 413,
        ATK_60107 = 414,
        ATK_60114 = 415,
        ATK_60203 = 416,
        ATK_60303 = 417,
        ATK_60309 = 471,
        ATK_60315 = 418,
        ATK_60325 = 419,
        ATK_60329 = 472,
        ATK_60404 = 420,
        ATK_60405 = 421,
        ATK_60415 = 422,
        ATK_60416 = 423,
        ATK_70002 = 424,
        ATK_70009 = 473,
        ATK_70015 = 425,
        ATK_70103 = 426,
        ATK_70114 = 427,
        ATK_70125 = 474,
        ATK_70203 = 428,
        ATK_80002 = 429,
        ATK_80004 = 430,
        ATK_80006 = 431,
        ATK_80103 = 432,
        ATK_80104 = 433,
        ATK_80116 = 434,
        ATK_80119 = 475,
        ATK_80124 = 435,
        ATK_80125 = 436,
        ATK_80135 = 476,
        ATK_80203 = 437,
        ATK_80204 = 438,
        ATK_80205 = 439,
        ATK_80206 = 440,
        ATK_80209 = 477,
        ATK_80303 = 441,
        ATK_80304 = 442,
        ATK_80305 = 443,
        ATK_80306 = 444,
        ATK_ActiveDecoy = 29,
        ATK_AnimalBodyAttack = 136,
        ATK_AntitankMine = 205,
        ATK_AntitankMine_G1 = 206,
        ATK_AntitankMine_G2 = 207,
        ATK_BattleGear_BodyAttack = 161,
        ATK_BattleGear_RailGun = 162,
        ATK_BearAttack = 138,
        ATK_BearRush = 139,
        ATK_BearStandAttack = 137,
        ATK_BlastWind = 288,
        ATK_Blow = 50,
        ATK_BombAssist = 212,
        ATK_BombAssistToSahelan = 213,
        ATK_BossQuiet_sr_010 = 293,
        ATK_BrokenBridge = 126,
        ATK_BrokenPillar = 15,
        ATK_C4 = 197,
        ATK_C4_G1 = 198,
        ATK_C4_G2 = 199,
        ATK_CBoxBodyAttack = 85,
        ATK_CBoxHeadAttack = 84,
        ATK_CannonShell = 11,
        ATK_CaptureCage = 33,
        ATK_CarryThrow = 56,
        ATK_CarryThrowBound = 3,
        ATK_ChaffAssist = 25,
        ATK_Claymore = 202,
        ATK_Claymore_G1 = 203,
        ATK_Claymore_G2 = 204,
        ATK_CqcChoke = 91,
        ATK_CqcContinuous2nd = 95,
        ATK_CqcContinuousOver3Times = 96,
        ATK_CqcDashBlastPunch = 99,
        ATK_CqcDashPunch = 98,
        ATK_CqcFinish = 89,
        ATK_CqcHang = 90,
        ATK_CqcHit = 8,
        ATK_CqcHitFinish = 88,
        ATK_CqcHoldThrow = 93,
        ATK_CqcKill = 97,
        ATK_CqcThrow = 92,
        ATK_CqcThrowBehind = 9,
        ATK_CqcThrowLadder = 10,
        ATK_CqcThrowWall = 94,
        ATK_CrawlSweep = 63,
        ATK_DDogBite = 144,
        ATK_DDogBodyAttack = 147,
        ATK_DDogKnife = 145,
        ATK_DDogStunKnife = 146,
        ATK_DMine = 202,
        ATK_DMine_G1 = 203,
        ATK_DMine_G2 = 204,
        ATK_DMine_G3 = 451,
        ATK_Death = 79,
        ATK_Decoy = 28,
        ATK_DecoyHit = 175,
        ATK_DecoyRaise = 200,
        ATK_DecoyShock = 176,
        ATK_Door = 120,
        ATK_DownKick = 83,
        ATK_Dung = 110,
        ATK_Dying = 77,
        ATK_EX_gl_000 = 447,
        ATK_EX_hg_000 = 446,
        ATK_EX_hg_010 = 478,
        ATK_EX_sr_000 = 448,
        ATK_ElectricWire = 129,
        ATK_ElectromagneticNetMine = 211,
        ATK_Event = 104,
        ATK_FOB_60325 = 449,
        ATK_Faint = 75,
        ATK_FaintRecover = 101,
        ATK_Fall = 103,
        ATK_FallBoundDeath = 59,
        ATK_FallBoundFaint = 58,
        ATK_FallDeath = 78,
        ATK_FireBurn = 121,
        ATK_FlashLight = 61,
        ATK_FlashLightAttack = 62,
        ATK_FlyingDisc = 34,
        ATK_FultonDevice = 201,
        ATK_GabageBoxOpen = 134,
        ATK_GimmickBlast = 123,
        ATK_GimmickBlastHigh = 124,
        ATK_Grenade = 178,
        ATK_GrenadeHit = 174,
        ATK_Grenade_G1 = 179,
        ATK_Grenade_G2 = 180,
        ATK_Grenade_G5 = 450,
        ATK_Grenade_G6 = 457,
        ATK_Grenade_G7 = 458,
        ATK_Grenade_G8 = 459,
        ATK_Grenader_Sleep = 318,
        ATK_Grenader_Smoke = 315,
        ATK_Grenader_Stun = 316,
        ATK_Grenader_Stun_Grazed = 317,
        ATK_GuardTower = 125,
        ATK_GunCameraAttack = 133,
        ATK_HeavyPhysics = 122,
        ATK_HeliBlast = 275,
        ATK_HeliChainGun = 274,
        ATK_HeliMiniGun = 269,
        ATK_HitEvadeAction = 55,
        ATK_HoneyBee = 310,
        ATK_HorseBodyAttack = 105,
        ATK_HorseBodyAttackDeath = 109,
        ATK_HorseBodyAttackMil1 = 106,
        ATK_HorseBodyAttackMil2 = 107,
        ATK_HorseBodyAttackMil3 = 108,
        ATK_InWater = 74,
        ATK_Jehuty = 166,
        ATK_Kibidango = 35,
        ATK_KibidangoHit = 177,
        ATK_Kick = 7,
        ATK_KillRocket = 170,
        ATK_KillRocket_G1 = 171,
        ATK_KillRocket_G2 = 172,
        ATK_Knife = 4,
        ATK_KnifeSlash = 53,
        ATK_KnifeSlashStun = 54,
        ATK_LargeFlame = 72,
        ATK_LifeRecover = 100,
        ATK_LightVehicleDrop = 119,
        ATK_LightVehicleHit = 113,
        ATK_LiquidWeaponThrow = 47,
        ATK_LiquidWeaponThrow_Machete = 258,
        ATK_LiquidWeaponThrow_Pipe = 259,
        ATK_Liquid_BottleThrow = 252,
        ATK_Liquid_Combo1 = 253,
        ATK_Liquid_Combo2 = 254,
        ATK_Liquid_Jump = 46,
        ATK_Liquid_Jump_Machete = 255,
        ATK_Liquid_Jump_Pipe = 256,
        ATK_Liquid_Kick = 260,
        ATK_Liquid_Thrust = 257,
        ATK_Machete = 5,
        ATK_MacheteDown = 6,
        ATK_Magazine = 173,
        ATK_MiddleFlame = 71,
        ATK_MolotovCocktail = 192,
        ATK_MolotovCocktailFire = 191,
        ATK_MolotovCocktailFire_G1 = 193,
        ATK_MolotovCocktailFire_G2 = 195,
        ATK_MolotovCocktail_G1 = 194,
        ATK_MolotovCocktail_G2 = 196,
        ATK_Mortar = 282,
        ATK_MortarSleep = 283,
        ATK_MsfPunch = 135,
        ATK_Nad = 264,
        ATK_Nad0 = 263,
        ATK_NadStun = 265,
        ATK_None = 0,
        ATK_NormalFlame = 69,
        ATK_ParasiteBeam = 37,
        ATK_ParasiteBlastWall = 223,
        ATK_ParasiteBreakWall = 215,
        ATK_ParasiteCamoufMachete = 219,
        ATK_ParasiteCorrode = 44,
        ATK_ParasiteCorrodeSmoke = 222,
        ATK_ParasiteFarAttack = 40,
        ATK_ParasiteGenerateWall = 216,
        ATK_ParasiteJumpAttack = 39,
        ATK_ParasiteJumpMachete = 220,
        ATK_ParasiteMachete = 218,
        ATK_ParasiteShove = 221,
        ATK_ParasiteThrowWall = 217,
        ATK_ParasiteVehicleBreak1 = 214,
        ATK_ParasiteVehicleBreak2 = 41,
        ATK_ParasiteVehicleBreak3 = 42,
        ATK_ParasiteVehicleBreak4 = 43,
        ATK_ParasiteWeaponBreak = 38,
        ATK_Pr_ar_010 = 295,
        ATK_Pr_sg_010 = 296,
        ATK_Pr_sm_010 = 294,
        ATK_Pr_sr_010 = 297,
        ATK_Punch = 80,
        ATK_Punch1 = 81,
        ATK_Punch2 = 82,
        ATK_Push = 48,
        ATK_PushPlayer = 51,
        ATK_PushSlide = 52,
        ATK_PushSlideInterp = 1,
        ATK_PushUp = 49,
        ATK_QuietDashKill = 141,
        ATK_QuietDashStun = 142,
        ATK_QuietDeath = 143,
        ATK_Quiet_sr_010 = 290,
        ATK_Quiet_sr_020 = 291,
        ATK_Quiet_sr_030 = 292,
        ATK_SP_hg_010 = 311,
        ATK_SP_hg_020 = 312,
        ATK_SP_sg_010 = 314,
        ATK_SP_sm_010 = 313,
        ATK_Sad = 266,
        ATK_SadStun = 267,
        ATK_Sahelan_AirMine = 244,
        ATK_Sahelan_Flame = 246,
        ATK_Sahelan_FootStamp = 236,
        ATK_Sahelan_Grenade = 245,
        ATK_Sahelan_HeadVulcan = 240,
        ATK_Sahelan_JumpImpact = 239,
        ATK_Sahelan_MidleMissile = 243,
        ATK_Sahelan_ParasiteSword = 241,
        ATK_Sahelan_PileBunker = 248,
        ATK_Sahelan_RailGun = 247,
        ATK_Sahelan_RockExplosion = 250,
        ATK_Sahelan_RodAttack = 237,
        ATK_Sahelan_RodAttackImpact = 238,
        ATK_Sahelan_SmallMissile = 242,
        ATK_Sahelan_Stomp = 249,
        ATK_Sahelan_TowerBreak = 251,
        ATK_ShieldBlock = 289,
        ATK_ShieldBroken = 73,
        ATK_ShieldSlash = 66,
        ATK_ShieldSlashStun = 67,
        ATK_ShockDecoy = 30,
        ATK_ShockDecoy_G1 = 31,
        ATK_ShockDecoy_G2 = 32,
        ATK_SkullFace_hg_010 = 309,
        ATK_Sleep = 76,
        ATK_SleepGus = 286,
        ATK_SleepGusAssist = 24,
        ATK_SleepGusOccurred = 287,
        ATK_SleepRecover = 102,
        ATK_SleepingGusGrenade = 188,
        ATK_SleepingGusGrenade_G1 = 189,
        ATK_SleepingGusGrenade_G2 = 190,
        ATK_SleepingGusMine = 208,
        ATK_SleepingGusMine_G1 = 209,
        ATK_SleepingGusMine_G2 = 210,
        ATK_SmallFlame = 70,
        ATK_Smoke = 284,
        ATK_SmokeAssist = 23,
        ATK_SmokeGrenade = 181,
        ATK_SmokeOccurred = 285,
        ATK_Stab = 2,
        ATK_StepOn = 65,
        ATK_Stomp = 60,
        ATK_StunArm = 163,
        ATK_StunArmThunder = 165,
        ATK_StunArm_G1 = 164,
        ATK_StunArm_G2 = 36,
        ATK_StunGrenade = 182,
        ATK_StunGrenadeGrazed = 185,
        ATK_StunGrenadeGrazed_G1 = 186,
        ATK_StunGrenadeGrazed_G2 = 187,
        ATK_StunGrenadeGrazed_G3 = 453,
        ATK_StunGrenadeGrazed_G4 = 461,
        ATK_StunGrenadeGrazed_G5 = 463,
        ATK_StunGrenadeGrazed_G6 = 465,
        ATK_StunGrenade_G1 = 183,
        ATK_StunGrenade_G2 = 184,
        ATK_StunGrenade_G3 = 452,
        ATK_StunGrenade_G4 = 460,
        ATK_StunGrenade_G5 = 462,
        ATK_StunGrenade_G6 = 464,
        ATK_StunRocket = 167,
        ATK_StunRocket_G1 = 168,
        ATK_StunRocket_G2 = 169,
        ATK_SupineFootSweep = 64,
        ATK_SupplyCBoxHit = 86,
        ATK_SupplyCBoxHitSpecial = 87,
        ATK_SupplyFlareGrenade = 27,
        ATK_SupportHeliFlareGrenade = 26,
        ATK_TankCannon = 13,
        ATK_Tankgun_105mmRifledBoreGun = 303,
        ATK_Tankgun_120mmSmoothBoreGun = 304,
        ATK_Tankgun_125mmSmoothBoreGun = 305,
        ATK_Tankgun_12_7mmHeavyMachineGun_East = 308,
        ATK_Tankgun_12_7mmHeavyMachineGun_West = 307,
        ATK_Tankgun_20mmAutoCannon = 301,
        ATK_Tankgun_30mmAutoCannon = 302,
        ATK_Tankgun_82mmRocketPoweredProjectile = 306,
        ATK_ThrowTackle = 68,
        ATK_Turret = 261,
        ATK_TurretStun = 262,
        ATK_UavExplosion = 273,
        ATK_UavGrenade = 271,
        ATK_UavLmg = 270,
        ATK_UavSleepGusGrenade = 272,
        ATK_VehicleBlast = 117,
        ATK_VehicleCrash = 116,
        ATK_VehicleDrop = 118,
        ATK_VehicleHit = 111,
        ATK_VehicleHitBreakObstacle = 115,
        ATK_VehicleHitMiddleSpeed = 114,
        ATK_VehiclePush = 112,
        ATK_VolRideBullet = 226,
        ATK_VolRideFirewall = 227,
        ATK_VolginBodyFire = 231,
        ATK_VolginBodyFireVehicle = 232,
        ATK_VolginBodyFireVehicleCrushed = 233,
        ATK_VolginChargeExplosion = 228,
        ATK_VolginFireBullet = 234,
        ATK_VolginFirePillar = 229,
        ATK_VolginFireball = 230,
        ATK_VolginGameOver = 45,
        ATK_VolginGroundFire = 235,
        ATK_WalkerGear_BodyAttack = 156,
        ATK_WalkerGear_BodyAttackBreakObstacle = 159,
        ATK_WalkerGear_FastBodyAttack = 157,
        ATK_WalkerGear_Flame = 150,
        ATK_WalkerGear_Fulton_Bomb = 149,
        ATK_WalkerGear_HandGun_00 = 18,
        ATK_WalkerGear_HandGun_01 = 19,
        ATK_WalkerGear_HandGun_02 = 20,
        ATK_WalkerGear_HandGun_03 = 21,
        ATK_WalkerGear_HandGun_04 = 22,
        ATK_WalkerGear_Kick = 160,
        ATK_WalkerGear_Machete_Slash = 152,
        ATK_WalkerGear_Machete_Stab = 153,
        ATK_WalkerGear_MiddleSpeedBodyAttack = 158,
        ATK_WalkerGear_MiniGun = 148,
        ATK_WalkerGear_Missile = 17,
        ATK_WalkerGear_Strike = 154,
        ATK_WalkerGear_StunGun = 151,
        ATK_WalkerGear_Throw = 155,
        ATK_WarHead = 57,
        ATK_WaterFaucetSplash = 16,
        ATK_WaterTankSplash = 128,
        ATK_WaterTowerSplash = 127,
        ATK_WaterWayArch = 130,
        ATK_WaterWayPillar = 132,
        ATK_WaterWayWall = 131,
        ATK_Wav1 = 268,
        ATK_WavCannon = 12,
        ATK_WavRocket = 14,
        ATK_WolfBite = 140,
        ATK_ZombieInfection = 224,
        ATK_ZombieScratch = 225,
        ATK_mgm0_ammo0 = 299,
        ATK_mgm0_famo0 = 300,
        ATK_mgm0_mgun0 = 298,
        ATK_uth0_ammo0 = 276,
        ATK_uth0_ammo0_2 = 277,
        ATK_uth0_ammo0_3 = 278,
        ATK_uth0_ammo1 = 279,
        ATK_uth0_ammo1_2 = 280,
        ATK_uth0_ammo1_3 = 281,
    },
    damSources = {
        DAM_SOURCE_Antimaterial = 9,
        DAM_SOURCE_Arm = 14,
        DAM_SOURCE_Assault = 3,
        DAM_SOURCE_Assist = 15,
        DAM_SOURCE_Boss = 22,
        DAM_SOURCE_Cqc = 12,
        DAM_SOURCE_CqcKnife = 13,
        DAM_SOURCE_DDog = 17,
        DAM_SOURCE_DHorse = 18,
        DAM_SOURCE_Grenader = 5,
        DAM_SOURCE_Handgun = 1,
        DAM_SOURCE_Heli = 20,
        DAM_SOURCE_Machinegun = 7,
        DAM_SOURCE_Missile = 8,
        DAM_SOURCE_None = 0,
        DAM_SOURCE_Placed = 11,
        DAM_SOURCE_Quiet = 16,
        DAM_SOURCE_Shotgun = 4,
        DAM_SOURCE_Sniper = 6,
        DAM_SOURCE_Submachinegun = 2,
        DAM_SOURCE_Throwing = 10,
        DAM_SOURCE_Vehicle = 19,
        DAM_SOURCE_WalkerGear = 21,
    },
    injParts ={
        INJ_PART_ALL = 1,
        INJ_PART_ARM = 2,
        INJ_PART_LEG = 3,
        INJ_PART_NONE = 0,  
    },
    injTypes = {
        INJ_TYPE_BULLET = 3,
        INJ_TYPE_CUT = 2,
        INJ_TYPE_DISLOCATED = 1,
        INJ_TYPE_NONE = 0,
    },
    eqpTypes = {
        EQP_TYPE_Ammo = 10,
        EQP_TYPE_AmmoBox = 15,
        EQP_TYPE_Assault = 1,
        EQP_TYPE_AttackArm = 14,
        EQP_TYPE_Bullet = 11,
        EQP_TYPE_GrenadeLauncher = 7,
        EQP_TYPE_Handgun = 2,
        EQP_TYPE_Item = 17,
        EQP_TYPE_Machinegun = 6,
        EQP_TYPE_Missile = 8,
        EQP_TYPE_None = 0,
        EQP_TYPE_Placed = 13,
        EQP_TYPE_Shield = 9,
        EQP_TYPE_Shotgun = 5,
        EQP_TYPE_Sniper = 4,
        EQP_TYPE_Submachinegun = 3,
        EQP_TYPE_Throwing = 12,
        EQP_TYPE_WeaponBox = 16,
    },
    reticleTypes = {
        RETICLE_UI_ASSAULT = 1,
        RETICLE_UI_EMPLACEMENT = 11,
        RETICLE_UI_GRENADER = 5,
        RETICLE_UI_HANDGUN = 7,
        RETICLE_UI_HANDGUN_TRANQ = 8,
        RETICLE_UI_MACHINEGUN = 4,
        RETICLE_UI_MISSILE = 6,
        RETICLE_UI_NONE = 0,
        RETICLE_UI_SHOTGUN = 3,
        RETICLE_UI_SNIPER = 2,
        RETICLE_UI_SUBMACHINEGUN = 9,
        RETICLE_UI_VEHICLE = 10,
    },
    triggerTypes = {
        TRIGGER_3PBURST = 2,
        TRIGGER_COCKING = 0,
        TRIGGER_FULLAUTO = 3,
        TRIGGER_SEMIAUTO = 1,
    },
    equipIDs = {
        EQP_WP_10001 = 560,
        EQP_WP_10002 = 561,
        EQP_WP_10003 = 562,
        EQP_WP_10004 = 563,
        EQP_WP_10006 = 564,
        EQP_WP_10015 = 565,
        EQP_WP_10024 = 566,
        EQP_WP_10025 = 567,
        EQP_WP_10035 = 568,
        EQP_WP_10036 = 569,
        EQP_WP_1003a = 1280,
        EQP_WP_1003b = 1281,
        EQP_WP_10101 = 570,
        EQP_WP_10102 = 571,
        EQP_WP_10103 = 572,
        EQP_WP_10104 = 573,
        EQP_WP_10105 = 574,
        EQP_WP_10107 = 575,
        EQP_WP_10116 = 576,
        EQP_WP_10125 = 577,
        EQP_WP_10134 = 578,
        EQP_WP_10136 = 579,
        EQP_WP_10201 = 580,
        EQP_WP_10202 = 581,
        EQP_WP_10203 = 582,
        EQP_WP_10205 = 583,
        EQP_WP_10214 = 584,
        EQP_WP_10216 = 585,
        EQP_WP_10302 = 586,
        EQP_WP_10303 = 587,
        EQP_WP_10304 = 588,
        EQP_WP_10305 = 589,
        EQP_WP_10306 = 590,
        EQP_WP_10307 = 591,
        EQP_WP_10403 = 592,
        EQP_WP_10404 = 593,
        EQP_WP_10405 = 594,
        EQP_WP_10407 = 595,
        EQP_WP_10414 = 596,
        EQP_WP_10415 = 597,
        EQP_WP_10417 = 598,
        EQP_WP_10424 = 599,
        EQP_WP_10425 = 600,
        EQP_WP_10427 = 601,
        EQP_WP_10503 = 602,
        EQP_WP_10504 = 603,
        EQP_WP_10515 = 604,
        EQP_WP_10526 = 605,
        EQP_WP_10603 = 606,
        EQP_WP_10604 = 607,
        EQP_WP_10615 = 608,
        EQP_WP_10626 = 609,
        EQP_WP_10637 = 610,
        EQP_WP_10703 = 611,
        EQP_WP_10704 = 612,
        EQP_WP_10705 = 613,
        EQP_WP_20002 = 614,
        EQP_WP_20003 = 615,
        EQP_WP_20004 = 616,
        EQP_WP_20005 = 617,
        EQP_WP_20006 = 618,
        EQP_WP_20015 = 619,
        EQP_WP_20103 = 620,
        EQP_WP_20104 = 621,
        EQP_WP_20105 = 622,
        EQP_WP_20106 = 623,
        EQP_WP_20116 = 624,
        EQP_WP_20119 = 1282,
        EQP_WP_2011a = 1283,
        EQP_WP_2011b = 1284,
        EQP_WP_20203 = 625,
        EQP_WP_20204 = 626,
        EQP_WP_20205 = 627,
        EQP_WP_20206 = 628,
        EQP_WP_20215 = 629,
        EQP_WP_20216 = 630,
        EQP_WP_20225 = 631,
        EQP_WP_20302 = 632,
        EQP_WP_20303 = 633,
        EQP_WP_20304 = 634,
        EQP_WP_20305 = 635,
        EQP_WP_20307 = 854,
        EQP_WP_20309 = 1285,
        EQP_WP_2030a = 1286,
        EQP_WP_2030b = 1287,
        EQP_WP_30001 = 637,
        EQP_WP_30002 = 638,
        EQP_WP_30003 = 639,
        EQP_WP_30004 = 640,
        EQP_WP_30005 = 641,
        EQP_WP_30014 = 642,
        EQP_WP_30015 = 643,
        EQP_WP_30016 = 644,
        EQP_WP_30023 = 645,
        EQP_WP_30024 = 646,
        EQP_WP_30025 = 647,
        EQP_WP_30034 = 648,
        EQP_WP_30035 = 649,
        EQP_WP_30036 = 650,
        EQP_WP_30043 = 651,
        EQP_WP_30044 = 652,
        EQP_WP_30045 = 653,
        EQP_WP_30046 = 654,
        EQP_WP_30047 = 655,
        EQP_WP_30054 = 656,
        EQP_WP_30055 = 657,
        EQP_WP_30056 = 658,
        EQP_WP_30057 = 659,
        EQP_WP_30101 = 660,
        EQP_WP_30102 = 661,
        EQP_WP_30103 = 662,
        EQP_WP_30104 = 663,
        EQP_WP_30105 = 664,
        EQP_WP_30113 = 665,
        EQP_WP_30114 = 666,
        EQP_WP_30115 = 667,
        EQP_WP_30117 = 855,
        EQP_WP_30119 = 1288,
        EQP_WP_3011a = 1289,
        EQP_WP_3011b = 1290,
        EQP_WP_30123 = 668,
        EQP_WP_30124 = 669,
        EQP_WP_30125 = 670,
        EQP_WP_30201 = 671,
        EQP_WP_30202 = 672,
        EQP_WP_30203 = 673,
        EQP_WP_30204 = 674,
        EQP_WP_30205 = 675,
        EQP_WP_30213 = 676,
        EQP_WP_30214 = 677,
        EQP_WP_30223 = 678,
        EQP_WP_30224 = 679,
        EQP_WP_30225 = 680,
        EQP_WP_30232 = 681,
        EQP_WP_30233 = 682,
        EQP_WP_30234 = 683,
        EQP_WP_30235 = 684,
        EQP_WP_30237 = 856,
        EQP_WP_30239 = 1291,
        EQP_WP_3023a = 1292,
        EQP_WP_3023b = 1293,
        EQP_WP_30303 = 685,
        EQP_WP_30304 = 686,
        EQP_WP_30305 = 687,
        EQP_WP_30306 = 688,
        EQP_WP_30314 = 689,
        EQP_WP_30315 = 690,
        EQP_WP_30316 = 691,
        EQP_WP_30325 = 692,
        EQP_WP_30326 = 693,
        EQP_WP_30327 = 694,
        EQP_WP_30334 = 695,
        EQP_WP_30335 = 696,
        EQP_WP_30336 = 697,
        EQP_WP_40001 = 698,
        EQP_WP_40002 = 699,
        EQP_WP_40003 = 700,
        EQP_WP_40004 = 701,
        EQP_WP_40012 = 702,
        EQP_WP_40013 = 703,
        EQP_WP_40014 = 704,
        EQP_WP_40015 = 705,
        EQP_WP_40023 = 706,
        EQP_WP_40024 = 707,
        EQP_WP_40025 = 708,
        EQP_WP_40032 = 709,
        EQP_WP_40033 = 710,
        EQP_WP_40034 = 711,
        EQP_WP_40035 = 712,
        EQP_WP_40042 = 713,
        EQP_WP_40043 = 714,
        EQP_WP_40044 = 715,
        EQP_WP_40045 = 716,
        EQP_WP_40102 = 717,
        EQP_WP_40103 = 718,
        EQP_WP_40104 = 719,
        EQP_WP_40105 = 720,
        EQP_WP_40106 = 721,
        EQP_WP_40115 = 722,
        EQP_WP_40116 = 723,
        EQP_WP_40123 = 851,
        EQP_WP_40124 = 852,
        EQP_WP_40125 = 853,
        EQP_WP_40126 = 724,
        EQP_WP_40127 = 725,
        EQP_WP_40133 = 726,
        EQP_WP_40134 = 727,
        EQP_WP_40135 = 728,
        EQP_WP_40136 = 729,
        EQP_WP_40143 = 730,
        EQP_WP_40144 = 731,
        EQP_WP_40203 = 732,
        EQP_WP_40204 = 733,
        EQP_WP_40205 = 734,
        EQP_WP_40206 = 735,
        EQP_WP_40207 = 736,
        EQP_WP_40304 = 737,
        EQP_WP_40305 = 738,
        EQP_WP_40306 = 739,
        EQP_WP_40307 = 740,
        EQP_WP_50002 = 741,
        EQP_WP_50003 = 742,
        EQP_WP_50004 = 743,
        EQP_WP_50005 = 744,
        EQP_WP_50015 = 745,
        EQP_WP_50026 = 746,
        EQP_WP_50033 = 748,
        EQP_WP_50034 = 749,
        EQP_WP_50035 = 750,
        EQP_WP_50036 = 751,
        EQP_WP_50047 = 747,
        EQP_WP_50102 = 752,
        EQP_WP_50103 = 753,
        EQP_WP_50104 = 754,
        EQP_WP_50105 = 755,
        EQP_WP_50115 = 756,
        EQP_WP_50126 = 757,
        EQP_WP_50133 = 759,
        EQP_WP_50134 = 760,
        EQP_WP_50135 = 761,
        EQP_WP_50136 = 762,
        EQP_WP_50147 = 758,
        EQP_WP_50202 = 763,
        EQP_WP_50203 = 764,
        EQP_WP_50204 = 765,
        EQP_WP_50215 = 766,
        EQP_WP_50226 = 767,
        EQP_WP_50237 = 768,
        EQP_WP_50303 = 769,
        EQP_WP_50304 = 770,
        EQP_WP_50305 = 771,
        EQP_WP_50306 = 772,
        EQP_WP_60001 = 773,
        EQP_WP_60002 = 774,
        EQP_WP_60003 = 775,
        EQP_WP_60004 = 776,
        EQP_WP_60005 = 777,
        EQP_WP_60012 = 778,
        EQP_WP_60013 = 779,
        EQP_WP_60015 = 780,
        EQP_WP_60016 = 781,
        EQP_WP_60102 = 782,
        EQP_WP_60103 = 783,
        EQP_WP_60104 = 784,
        EQP_WP_60105 = 785,
        EQP_WP_60106 = 786,
        EQP_WP_60107 = 787,
        EQP_WP_60114 = 788,
        EQP_WP_60115 = 789,
        EQP_WP_60116 = 790,
        EQP_WP_60117 = 791,
        EQP_WP_60202 = 792,
        EQP_WP_60203 = 793,
        EQP_WP_60204 = 794,
        EQP_WP_60205 = 795,
        EQP_WP_60206 = 796,
        EQP_WP_60303 = 797,
        EQP_WP_60304 = 798,
        EQP_WP_60305 = 799,
        EQP_WP_60306 = 800,
        EQP_WP_60309 = 1294,
        EQP_WP_6030a = 1295,
        EQP_WP_6030b = 1296,
        EQP_WP_60315 = 801,
        EQP_WP_60316 = 802,
        EQP_WP_60317 = 803,
        EQP_WP_60325 = 804,
        EQP_WP_60326 = 805,
        EQP_WP_60327 = 806,
        EQP_WP_60329 = 1297,
        EQP_WP_6032a = 1298,
        EQP_WP_6032b = 1299,
        EQP_WP_60404 = 807,
        EQP_WP_60405 = 808,
        EQP_WP_60406 = 809,
        EQP_WP_60407 = 810,
        EQP_WP_6040a = 1300,
        EQP_WP_60415 = 811,
        EQP_WP_60416 = 812,
        EQP_WP_60417 = 813,
        EQP_WP_70002 = 814,
        EQP_WP_70003 = 815,
        EQP_WP_70004 = 816,
        EQP_WP_70005 = 817,
        EQP_WP_70006 = 818,
        EQP_WP_70009 = 1301,
        EQP_WP_7000a = 1302,
        EQP_WP_7000b = 1303,
        EQP_WP_70015 = 819,
        EQP_WP_70024 = 820,
        EQP_WP_70025 = 821,
        EQP_WP_70103 = 822,
        EQP_WP_70104 = 823,
        EQP_WP_70105 = 824,
        EQP_WP_70106 = 825,
        EQP_WP_70114 = 826,
        EQP_WP_70115 = 827,
        EQP_WP_70116 = 828,
        EQP_WP_70125 = 1304,
        EQP_WP_70126 = 1305,
        EQP_WP_70127 = 1306,
        EQP_WP_70203 = 829,
        EQP_WP_70204 = 830,
        EQP_WP_70205 = 831,
        EQP_WP_70206 = 832,
        EQP_WP_80002 = 833,
        EQP_WP_80004 = 834,
        EQP_WP_80006 = 835,
        EQP_WP_80103 = 836,
        EQP_WP_80104 = 837,
        EQP_WP_80105 = 838,
        EQP_WP_80116 = 839,
        EQP_WP_80119 = 1307,
        EQP_WP_8011a = 1308,
        EQP_WP_8011b = 1309,
        EQP_WP_80124 = 840,
        EQP_WP_80125 = 841,
        EQP_WP_80126 = 842,
        EQP_WP_80135 = 1310,
        EQP_WP_80136 = 1311,
        EQP_WP_80138 = 1312,
        EQP_WP_8013a = 1313,
        EQP_WP_8013b = 1314,
        EQP_WP_80203 = 843,
        EQP_WP_80204 = 844,
        EQP_WP_80205 = 845,
        EQP_WP_80206 = 846,
        EQP_WP_80209 = 1315,
        EQP_WP_8020a = 1316,
        EQP_WP_8020b = 1317,
        EQP_WP_80304 = 847,
        EQP_WP_80305 = 848,
        EQP_WP_80306 = 849,
        EQP_WP_80307 = 850,
        EQP_WP_BossQuiet_sr_010 = 62,
        EQP_WP_Com_ms_010 = 52,
        EQP_WP_Com_ms_020 = 53,
        EQP_WP_Com_ms_023 = 1044,
        EQP_WP_Com_ms_024 = 1045,
        EQP_WP_Com_ms_026 = 1058,
        EQP_WP_Com_ms_029 = 1095,
        EQP_WP_Com_ms_02a = 1096,
        EQP_WP_Com_ms_02b = 1097,
        EQP_WP_Com_sg_010 = 21,
        EQP_WP_Com_sg_011 = 22,
        EQP_WP_Com_sg_011_FL = 23,
        EQP_WP_Com_sg_013 = 1036,
        EQP_WP_Com_sg_015 = 1037,
        EQP_WP_Com_sg_016 = 1052,
        EQP_WP_Com_sg_018 = 1053,
        EQP_WP_Com_sg_020 = 24,
        EQP_WP_Com_sg_020_FL = 25,
        EQP_WP_Com_sg_023 = 1024,
        EQP_WP_Com_sg_024 = 1025,
        EQP_WP_Com_sg_025 = 1026,
        EQP_WP_Com_sg_030 = 26,
        EQP_WP_Com_sg_038 = 1051,
        EQP_WP_DEBUG_sr_010 = 72,
        EQP_WP_DEMO_ar_010 = 73,
        EQP_WP_DEMO_ar_020 = 74,
        EQP_WP_DEMO_ar_030 = 75,
        EQP_WP_DEMO_hg_010 = 77,
        EQP_WP_DEMO_hg_020 = 78,
        EQP_WP_DEMO_hg_030 = 79,
        EQP_WP_DEMO_mg_010 = 82,
        EQP_WP_DEMO_ms_010 = 83,
        EQP_WP_DEMO_ms_020 = 84,
        EQP_WP_DEMO_sm_010 = 80,
        EQP_WP_DEMO_sm_020 = 81,
        EQP_WP_DEMO_sr_010 = 76,
        EQP_WP_EX_gl_000 = 1047,
        EQP_WP_EX_hg_000 = 1046,
        EQP_WP_EX_hg_000_G01 = 1063,
        EQP_WP_EX_hg_000_G02 = 1064,
        EQP_WP_EX_hg_000_G03 = 1065,
        EQP_WP_EX_hg_000_G04 = 1066,
        EQP_WP_EX_hg_010 = 1067,
        EQP_WP_EX_hg_011 = 1068,
        EQP_WP_EX_hg_012 = 1069,
        EQP_WP_EX_hg_013 = 1070,
        EQP_WP_EX_sr_000 = 1048,
        EQP_WP_East_ar_010 = 36,
        EQP_WP_East_ar_010_FL = 37,
        EQP_WP_East_ar_020 = 38,
        EQP_WP_East_ar_030 = 39,
        EQP_WP_East_ar_030_FL = 40,
        EQP_WP_East_hg_010 = 6,
        EQP_WP_East_mg_010 = 51,
        EQP_WP_East_ms_010 = 56,
        EQP_WP_East_ms_020 = 57,
        EQP_WP_East_sm_010 = 15,
        EQP_WP_East_sm_020 = 16,
        EQP_WP_East_sm_030 = 17,
        EQP_WP_East_sm_042 = 18,
        EQP_WP_East_sm_043 = 1027,
        EQP_WP_East_sm_044 = 1028,
        EQP_WP_East_sm_045 = 1029,
        EQP_WP_East_sm_047 = 1059,
        EQP_WP_East_sm_049 = 1074,
        EQP_WP_East_sm_04a = 1075,
        EQP_WP_East_sm_04b = 1076,
        EQP_WP_East_sr_011 = 45,
        EQP_WP_East_sr_020 = 46,
        EQP_WP_East_sr_032 = 86,
        EQP_WP_East_sr_033 = 1038,
        EQP_WP_East_sr_034 = 1039,
        EQP_WP_HoneyBee = 69,
        EQP_WP_Pr_ar_010 = 64,
        EQP_WP_Pr_sg_010 = 65,
        EQP_WP_Pr_sm_010 = 63,
        EQP_WP_Pr_sr_010 = 66,
        EQP_WP_Quiet_sr_010 = 59,
        EQP_WP_Quiet_sr_020 = 60,
        EQP_WP_Quiet_sr_030 = 61,
        EQP_WP_SCamLocator = 1062,
        EQP_WP_SP_SLD_010 = 105,
        EQP_WP_SP_SLD_010_G01 = 106,
        EQP_WP_SP_SLD_010_G02 = 107,
        EQP_WP_SP_SLD_020 = 108,
        EQP_WP_SP_SLD_020_G01 = 109,
        EQP_WP_SP_SLD_020_G02 = 110,
        EQP_WP_SP_SLD_030 = 111,
        EQP_WP_SP_SLD_030_G01 = 112,
        EQP_WP_SP_SLD_030_G02 = 113,
        EQP_WP_SP_SLD_040 = 114,
        EQP_WP_SP_SLD_040_G01 = 115,
        EQP_WP_SP_SLD_040_G02 = 116,
        EQP_WP_SP_hg_010 = 101,
        EQP_WP_SP_hg_020 = 102,
        EQP_WP_SP_sg_010 = 104,
        EQP_WP_SP_sm_010 = 103,
        EQP_WP_SkullFace_hg_010 = 71,
        EQP_WP_Volgin_sg_010 = 70,
        EQP_WP_West_ar_010 = 27,
        EQP_WP_West_ar_010_FL = 28,
        EQP_WP_West_ar_020 = 29,
        EQP_WP_West_ar_020_FL = 30,
        EQP_WP_West_ar_030 = 31,
        EQP_WP_West_ar_040 = 32,
        EQP_WP_West_ar_042 = 1032,
        EQP_WP_West_ar_050 = 33,
        EQP_WP_West_ar_055 = 1033,
        EQP_WP_West_ar_057 = 1060,
        EQP_WP_West_ar_059 = 1077,
        EQP_WP_West_ar_05a = 1078,
        EQP_WP_West_ar_05b = 1079,
        EQP_WP_West_ar_060 = 34,
        EQP_WP_West_ar_063 = 1034,
        EQP_WP_West_ar_070 = 35,
        EQP_WP_West_ar_075 = 1035,
        EQP_WP_West_ar_077 = 1061,
        EQP_WP_West_ar_079 = 1080,
        EQP_WP_West_ar_07a = 1081,
        EQP_WP_West_ar_07b = 1082,
        EQP_WP_West_hg_010 = 1,
        EQP_WP_West_hg_010_WG = 2,
        EQP_WP_West_hg_020 = 3,
        EQP_WP_West_hg_030 = 4,
        EQP_WP_West_hg_030_cmn = 5,
        EQP_WP_West_mg_010 = 47,
        EQP_WP_West_mg_020 = 48,
        EQP_WP_West_mg_021 = 49,
        EQP_WP_West_mg_023 = 1042,
        EQP_WP_West_mg_024 = 1043,
        EQP_WP_West_mg_030 = 50,
        EQP_WP_West_mg_037 = 1057,
        EQP_WP_West_mg_039 = 1089,
        EQP_WP_West_mg_03a = 1090,
        EQP_WP_West_mg_03b = 1091,
        EQP_WP_West_ms_010 = 54,
        EQP_WP_West_ms_020 = 55,
        EQP_WP_West_ms_029 = 1092,
        EQP_WP_West_ms_02a = 1093,
        EQP_WP_West_ms_02b = 1094,
        EQP_WP_West_sm_010 = 12,
        EQP_WP_West_sm_010_WG = 13,
        EQP_WP_West_sm_014 = 1030,
        EQP_WP_West_sm_015 = 1031,
        EQP_WP_West_sm_016 = 1049,
        EQP_WP_West_sm_017 = 1050,
        EQP_WP_West_sm_019 = 1071,
        EQP_WP_West_sm_01a = 1072,
        EQP_WP_West_sm_01b = 1073,
        EQP_WP_West_sm_020 = 14,
        EQP_WP_West_sr_010 = 41,
        EQP_WP_West_sr_011 = 42,
        EQP_WP_West_sr_013 = 1040,
        EQP_WP_West_sr_014 = 1041,
        EQP_WP_West_sr_020 = 43,
        EQP_WP_West_sr_027 = 1054,
        EQP_WP_West_sr_029 = 1083,
        EQP_WP_West_sr_02a = 1084,
        EQP_WP_West_sr_02b = 1085,
        EQP_WP_West_sr_037 = 85,
        EQP_WP_West_sr_047 = 1055,
        EQP_WP_West_sr_048 = 1056,
        EQP_WP_West_sr_049 = 1086,
        EQP_WP_West_sr_04a = 1087,
        EQP_WP_West_sr_04b = 1088,
        EQP_WP_West_thg_010 = 7,
        EQP_WP_West_thg_020 = 8,
        EQP_WP_West_thg_030 = 9,
        EQP_WP_West_thg_040 = 10,
        EQP_WP_West_thg_050 = 11,
        EQP_WP_Wood_ar_010 = 58,
        EQP_WP_mgm0_cmn_ammo1 = 68,
        EQP_WP_mgm0_mgun0 = 67,
        EQP_WP_no_use_00 = 636,
        EQP_WP_no_use_01 = 44,
        EQP_WP_no_use_02 = 19,
        EQP_WP_no_use_03 = 20,
    },
    wpIDs = {
        WP_10001 = 1,
        WP_10002 = 2,
        WP_10003 = 3,
        WP_10004 = 4,
        WP_10006 = 5,
        WP_10015 = 6,
        WP_10024 = 7,
        WP_10025 = 8,
        WP_10035 = 9,
        WP_10036 = 10,
        WP_1003a = 312,
        WP_1003b = 313,
        WP_10101 = 11,
        WP_10102 = 12,
        WP_10103 = 13,
        WP_10104 = 14,
        WP_10105 = 15,
        WP_10107 = 16,
        WP_10116 = 17,
        WP_10125 = 18,
        WP_10134 = 19,
        WP_10136 = 20,
        WP_10201 = 21,
        WP_10202 = 22,
        WP_10203 = 23,
        WP_10205 = 24,
        WP_10214 = 25,
        WP_10216 = 26,
        WP_10302 = 27,
        WP_10303 = 28,
        WP_10304 = 29,
        WP_10305 = 30,
        WP_10306 = 31,
        WP_10307 = 32,
        WP_10403 = 33,
        WP_10404 = 34,
        WP_10405 = 35,
        WP_10407 = 36,
        WP_10414 = 37,
        WP_10415 = 38,
        WP_10417 = 39,
        WP_10424 = 40,
        WP_10425 = 41,
        WP_10427 = 42,
        WP_10503 = 43,
        WP_10504 = 44,
        WP_10515 = 45,
        WP_10526 = 46,
        WP_10603 = 47,
        WP_10604 = 48,
        WP_10615 = 49,
        WP_10626 = 50,
        WP_10637 = 51,
        WP_10703 = 52,
        WP_10704 = 53,
        WP_10705 = 54,
        WP_20002 = 55,
        WP_20003 = 56,
        WP_20004 = 57,
        WP_20005 = 58,
        WP_20006 = 59,
        WP_20015 = 60,
        WP_20103 = 61,
        WP_20104 = 62,
        WP_20105 = 63,
        WP_20106 = 64,
        WP_20116 = 65,
        WP_20119 = 314,
        WP_2011a = 315,
        WP_2011b = 316,
        WP_20203 = 66,
        WP_20204 = 67,
        WP_20205 = 68,
        WP_20206 = 69,
        WP_20215 = 70,
        WP_20216 = 71,
        WP_20225 = 72,
        WP_20302 = 73,
        WP_20303 = 74,
        WP_20304 = 75,
        WP_20305 = 76,
        WP_20306 = 77,
        WP_20307 = 295,
        WP_20309 = 317,
        WP_2030a = 318,
        WP_2030b = 319,
        WP_30001 = 78,
        WP_30002 = 79,
        WP_30003 = 80,
        WP_30004 = 81,
        WP_30005 = 82,
        WP_30014 = 83,
        WP_30015 = 84,
        WP_30016 = 85,
        WP_30023 = 86,
        WP_30024 = 87,
        WP_30025 = 88,
        WP_30034 = 89,
        WP_30035 = 90,
        WP_30036 = 91,
        WP_30043 = 92,
        WP_30044 = 93,
        WP_30045 = 94,
        WP_30046 = 95,
        WP_30047 = 96,
        WP_30054 = 97,
        WP_30055 = 98,
        WP_30056 = 99,
        WP_30057 = 100,
        WP_30101 = 101,
        WP_30102 = 102,
        WP_30103 = 103,
        WP_30104 = 104,
        WP_30105 = 105,
        WP_30113 = 106,
        WP_30114 = 107,
        WP_30115 = 108,
        WP_30117 = 296,
        WP_30119 = 320,
        WP_3011a = 321,
        WP_3011b = 322,
        WP_30123 = 109,
        WP_30124 = 110,
        WP_30125 = 111,
        WP_30201 = 112,
        WP_30202 = 113,
        WP_30203 = 114,
        WP_30204 = 115,
        WP_30205 = 116,
        WP_30213 = 117,
        WP_30214 = 118,
        WP_30223 = 119,
        WP_30224 = 120,
        WP_30225 = 121,
        WP_30232 = 122,
        WP_30233 = 123,
        WP_30234 = 124,
        WP_30235 = 125,
        WP_30237 = 297,
        WP_30239 = 323,
        WP_3023a = 324,
        WP_3023b = 325,
        WP_30303 = 126,
        WP_30304 = 127,
        WP_30305 = 128,
        WP_30306 = 129,
        WP_30314 = 130,
        WP_30315 = 131,
        WP_30316 = 132,
        WP_30325 = 133,
        WP_30326 = 134,
        WP_30327 = 135,
        WP_30334 = 136,
        WP_30335 = 137,
        WP_30336 = 138,
        WP_40001 = 139,
        WP_40002 = 140,
        WP_40003 = 141,
        WP_40004 = 142,
        WP_40012 = 143,
        WP_40013 = 144,
        WP_40014 = 145,
        WP_40015 = 146,
        WP_40023 = 147,
        WP_40024 = 148,
        WP_40025 = 149,
        WP_40032 = 150,
        WP_40033 = 151,
        WP_40034 = 152,
        WP_40035 = 153,
        WP_40042 = 154,
        WP_40043 = 155,
        WP_40044 = 156,
        WP_40045 = 157,
        WP_40102 = 158,
        WP_40103 = 159,
        WP_40104 = 160,
        WP_40105 = 161,
        WP_40106 = 162,
        WP_40115 = 163,
        WP_40116 = 164,
        WP_40123 = 292,
        WP_40124 = 293,
        WP_40125 = 294,
        WP_40126 = 165,
        WP_40127 = 166,
        WP_40133 = 167,
        WP_40134 = 168,
        WP_40135 = 169,
        WP_40136 = 170,
        WP_40143 = 171,
        WP_40144 = 172,
        WP_40203 = 173,
        WP_40204 = 174,
        WP_40205 = 175,
        WP_40206 = 176,
        WP_40207 = 177,
        WP_40304 = 178,
        WP_40305 = 179,
        WP_40306 = 180,
        WP_40307 = 181,
        WP_50002 = 182,
        WP_50003 = 183,
        WP_50004 = 184,
        WP_50005 = 185,
        WP_50015 = 186,
        WP_50026 = 187,
        WP_50033 = 189,
        WP_50034 = 190,
        WP_50035 = 191,
        WP_50036 = 192,
        WP_50047 = 188,
        WP_50102 = 193,
        WP_50103 = 194,
        WP_50104 = 195,
        WP_50105 = 196,
        WP_50115 = 197,
        WP_50126 = 198,
        WP_50133 = 200,
        WP_50134 = 201,
        WP_50135 = 202,
        WP_50136 = 203,
        WP_50147 = 199,
        WP_50202 = 204,
        WP_50203 = 205,
        WP_50204 = 206,
        WP_50215 = 207,
        WP_50226 = 208,
        WP_50237 = 209,
        WP_50303 = 210,
        WP_50304 = 211,
        WP_50305 = 212,
        WP_50306 = 213,
        WP_60001 = 214,
        WP_60002 = 215,
        WP_60003 = 216,
        WP_60004 = 217,
        WP_60005 = 218,
        WP_60012 = 219,
        WP_60013 = 220,
        WP_60015 = 221,
        WP_60016 = 222,
        WP_60102 = 223,
        WP_60103 = 224,
        WP_60104 = 225,
        WP_60105 = 226,
        WP_60106 = 227,
        WP_60107 = 228,
        WP_60114 = 229,
        WP_60115 = 230,
        WP_60116 = 231,
        WP_60117 = 232,
        WP_60202 = 233,
        WP_60203 = 234,
        WP_60204 = 235,
        WP_60205 = 236,
        WP_60206 = 237,
        WP_60303 = 238,
        WP_60304 = 239,
        WP_60305 = 240,
        WP_60306 = 241,
        WP_60309 = 326,
        WP_6030a = 327,
        WP_6030b = 328,
        WP_60315 = 242,
        WP_60316 = 243,
        WP_60317 = 244,
        WP_60325 = 245,
        WP_60326 = 246,
        WP_60327 = 247,
        WP_60329 = 329,
        WP_6032a = 330,
        WP_6032b = 331,
        WP_60404 = 248,
        WP_60405 = 249,
        WP_60406 = 250,
        WP_60407 = 251,
        WP_6040a = 332,
        WP_60415 = 252,
        WP_60416 = 253,
        WP_60417 = 254,
        WP_70002 = 255,
        WP_70003 = 256,
        WP_70004 = 257,
        WP_70005 = 258,
        WP_70006 = 259,
        WP_70009 = 333,
        WP_7000a = 334,
        WP_7000b = 335,
        WP_70015 = 260,
        WP_70024 = 261,
        WP_70025 = 262,
        WP_70103 = 263,
        WP_70104 = 264,
        WP_70105 = 265,
        WP_70106 = 266,
        WP_70114 = 267,
        WP_70115 = 268,
        WP_70116 = 269,
        WP_70125 = 336,
        WP_70126 = 337,
        WP_70127 = 338,
        WP_70203 = 270,
        WP_70204 = 271,
        WP_70205 = 272,
        WP_70206 = 273,
        WP_80002 = 274,
        WP_80004 = 275,
        WP_80006 = 276,
        WP_80103 = 277,
        WP_80104 = 278,
        WP_80105 = 279,
        WP_80116 = 280,
        WP_80119 = 339,
        WP_8011a = 340,
        WP_8011b = 341,
        WP_80124 = 281,
        WP_80125 = 282,
        WP_80126 = 283,
        WP_80135 = 342,
        WP_80136 = 343,
        WP_80138 = 344,
        WP_8013a = 345,
        WP_8013b = 346,
        WP_80203 = 284,
        WP_80204 = 285,
        WP_80205 = 286,
        WP_80206 = 287,
        WP_80209 = 347,
        WP_8020a = 348,
        WP_8020b = 349,
        WP_80304 = 288,
        WP_80305 = 289,
        WP_80306 = 290,
        WP_80307 = 291,
        WP_BossQuiet_sr_010 = 410,
        WP_Com_ms_010 = 400,
        WP_Com_ms_020 = 401,
        WP_Com_ms_023 = 455,
        WP_Com_ms_024 = 456,
        WP_Com_ms_026 = 469,
        WP_Com_ms_029 = 506,
        WP_Com_ms_02a = 507,
        WP_Com_ms_02b = 508,
        WP_Com_sg_010 = 367,
        WP_Com_sg_011 = 368,
        WP_Com_sg_011_FL = 369,
        WP_Com_sg_013 = 447,
        WP_Com_sg_015 = 448,
        WP_Com_sg_016 = 463,
        WP_Com_sg_018 = 464,
        WP_Com_sg_020 = 370,
        WP_Com_sg_020_FL = 371,
        WP_Com_sg_023 = 435,
        WP_Com_sg_024 = 436,
        WP_Com_sg_025 = 437,
        WP_Com_sg_030 = 372,
        WP_Com_sg_038 = 462,
        WP_DEMO_ar_010 = 418,
        WP_DEMO_ar_020 = 419,
        WP_DEMO_ar_030 = 420,
        WP_DEMO_hg_010 = 422,
        WP_DEMO_hg_020 = 423,
        WP_DEMO_hg_030 = 424,
        WP_DEMO_mg_010 = 427,
        WP_DEMO_ms_010 = 428,
        WP_DEMO_ms_020 = 429,
        WP_DEMO_sm_010 = 425,
        WP_DEMO_sm_020 = 426,
        WP_DEMO_sr_010 = 421,
        WP_EX_gl_000 = 458,
        WP_EX_hg_000 = 457,
        WP_EX_hg_000_G01 = 474,
        WP_EX_hg_000_G02 = 475,
        WP_EX_hg_000_G03 = 476,
        WP_EX_hg_000_G04 = 477,
        WP_EX_hg_010 = 478,
        WP_EX_hg_011 = 479,
        WP_EX_hg_012 = 480,
        WP_EX_hg_013 = 481,
        WP_EX_sr_000 = 459,
        WP_East_ar_010 = 382,
        WP_East_ar_010_FL = 383,
        WP_East_ar_020 = 384,
        WP_East_ar_030 = 385,
        WP_East_ar_030_FL = 386,
        WP_East_hg_010 = 354,
        WP_East_mg_010 = 399,
        WP_East_ms_010 = 404,
        WP_East_ms_020 = 405,
        WP_East_sm_010 = 363,
        WP_East_sm_020 = 364,
        WP_East_sm_030 = 365,
        WP_East_sm_042 = 366,
        WP_East_sm_043 = 438,
        WP_East_sm_044 = 439,
        WP_East_sm_045 = 440,
        WP_East_sm_047 = 470,
        WP_East_sm_049 = 485,
        WP_East_sm_04a = 486,
        WP_East_sm_04b = 487,
        WP_East_sr_010 = 391,
        WP_East_sr_011 = 392,
        WP_East_sr_020 = 393,
        WP_East_sr_032 = 394,
        WP_East_sr_033 = 449,
        WP_East_sr_034 = 450,
        WP_HoneyBee = 434,
        WP_None = 0,
        WP_Pr_ar_010 = 412,
        WP_Pr_sg_010 = 413,
        WP_Pr_sm_010 = 411,
        WP_Pr_sr_010 = 414,
        WP_Quiet_sr_010 = 407,
        WP_Quiet_sr_020 = 408,
        WP_Quiet_sr_030 = 409,
        WP_SCamLocator = 473,
        WP_SP_hg_010 = 430,
        WP_SP_hg_020 = 431,
        WP_SP_sg_010 = 433,
        WP_SP_sm_010 = 432,
        WP_SkullFace_hg_010 = 417,
        WP_Volgin_sg_010 = 406,
        WP_West_ar_010 = 373,
        WP_West_ar_010_FL = 374,
        WP_West_ar_020 = 375,
        WP_West_ar_020_FL = 376,
        WP_West_ar_030 = 377,
        WP_West_ar_040 = 378,
        WP_West_ar_042 = 443,
        WP_West_ar_050 = 379,
        WP_West_ar_055 = 444,
        WP_West_ar_057 = 471,
        WP_West_ar_059 = 488,
        WP_West_ar_05a = 489,
        WP_West_ar_05b = 490,
        WP_West_ar_060 = 380,
        WP_West_ar_063 = 445,
        WP_West_ar_070 = 381,
        WP_West_ar_075 = 446,
        WP_West_ar_077 = 472,
        WP_West_ar_079 = 491,
        WP_West_ar_07a = 492,
        WP_West_ar_07b = 493,
        WP_West_hg_010 = 350,
        WP_West_hg_010_WG = 351,
        WP_West_hg_020 = 352,
        WP_West_hg_030 = 353,
        WP_West_mg_010 = 395,
        WP_West_mg_020 = 396,
        WP_West_mg_021 = 397,
        WP_West_mg_023 = 453,
        WP_West_mg_024 = 454,
        WP_West_mg_030 = 398,
        WP_West_mg_037 = 468,
        WP_West_mg_039 = 500,
        WP_West_mg_03a = 501,
        WP_West_mg_03b = 502,
        WP_West_ms_010 = 402,
        WP_West_ms_020 = 403,
        WP_West_ms_029 = 503,
        WP_West_ms_02a = 504,
        WP_West_ms_02b = 505,
        WP_West_sm_010 = 360,
        WP_West_sm_010_WG = 361,
        WP_West_sm_014 = 441,
        WP_West_sm_015 = 442,
        WP_West_sm_016 = 460,
        WP_West_sm_017 = 461,
        WP_West_sm_019 = 482,
        WP_West_sm_01a = 483,
        WP_West_sm_01b = 484,
        WP_West_sm_020 = 362,
        WP_West_sr_010 = 387,
        WP_West_sr_011 = 388,
        WP_West_sr_013 = 451,
        WP_West_sr_014 = 452,
        WP_West_sr_020 = 389,
        WP_West_sr_027 = 465,
        WP_West_sr_029 = 494,
        WP_West_sr_02a = 495,
        WP_West_sr_02b = 496,
        WP_West_sr_037 = 390,
        WP_West_sr_047 = 466,
        WP_West_sr_048 = 467,
        WP_West_sr_049 = 497,
        WP_West_sr_04a = 498,
        WP_West_sr_04b = 499,
        WP_West_thg_010 = 355,
        WP_West_thg_020 = 356,
        WP_West_thg_030 = 357,
        WP_West_thg_040 = 358,
        WP_West_thg_050 = 359,
        WP_Wood_ar_010 = 416,
        WP_mgm0_mgun0 = 415
    },
    recvIDs = {
        RC_10001 = 1,
        RC_10003 = 2,
        RC_10004 = 3,
        RC_10006 = 4,
        RC_10006_u = 5,
        RC_10015 = 6,
        RC_10024 = 7,
        RC_10035 = 8,
        RC_1003a = 200,
        RC_10101 = 9,
        RC_10102 = 10,
        RC_10105 = 11,
        RC_10107 = 12,
        RC_10107_u = 13,
        RC_10117 = 14,
        RC_10125 = 15,
        RC_10134 = 16,
        RC_10136 = 17,
        RC_10201 = 18,
        RC_10203 = 19,
        RC_10205 = 20,
        RC_10214 = 21,
        RC_10216 = 22,
        RC_10302 = 23,
        RC_10303 = 24,
        RC_10304 = 25,
        RC_10306 = 26,
        RC_10307 = 27,
        RC_10307_u = 28,
        RC_10403 = 29,
        RC_10405 = 30,
        RC_10407 = 31,
        RC_10503 = 32,
        RC_10504 = 33,
        RC_10515 = 34,
        RC_10526 = 35,
        RC_10603 = 36,
        RC_10604 = 37,
        RC_10615 = 38,
        RC_10626 = 39,
        RC_10637 = 40,
        RC_10703 = 41,
        RC_10704 = 42,
        RC_10705 = 43,
        RC_20002 = 44,
        RC_20004 = 45,
        RC_20006 = 46,
        RC_20015 = 47,
        RC_20103 = 48,
        RC_20104 = 49,
        RC_20105 = 50,
        RC_20116 = 51,
        RC_20119 = 201,
        RC_20203 = 52,
        RC_20204 = 53,
        RC_20205 = 54,
        RC_20215 = 55,
        RC_20225 = 56,
        RC_20302 = 57,
        RC_20303 = 58,
        RC_20304 = 59,
        RC_20307 = 197,
        RC_20309 = 202,
        RC_30001 = 61,
        RC_30003 = 62,
        RC_30005 = 63,
        RC_30014 = 64,
        RC_30016 = 65,
        RC_30034 = 66,
        RC_30101 = 67,
        RC_30102 = 68,
        RC_30104 = 69,
        RC_30117 = 198,
        RC_30119 = 203,
        RC_30201 = 70,
        RC_30202 = 71,
        RC_30232 = 72,
        RC_30233 = 73,
        RC_30235 = 74,
        RC_30237 = 199,
        RC_30239 = 204,
        RC_30303 = 75,
        RC_30305 = 76,
        RC_30306 = 77,
        RC_30325 = 78,
        RC_30327 = 79,
        RC_40001 = 80,
        RC_40002 = 81,
        RC_40003 = 82,
        RC_40004 = 83,
        RC_40012 = 84,
        RC_40013 = 85,
        RC_40014 = 86,
        RC_40015 = 87,
        RC_40023 = 88,
        RC_40024 = 89,
        RC_40025 = 90,
        RC_40032 = 91,
        RC_40033 = 92,
        RC_40034 = 93,
        RC_40035 = 94,
        RC_40042 = 95,
        RC_40042_u = 96,
        RC_40044 = 97,
        RC_40045 = 98,
        RC_40045_u = 99,
        RC_40102 = 100,
        RC_40102_u = 101,
        RC_40103 = 102,
        RC_40105 = 103,
        RC_40115 = 104,
        RC_40123 = 196,
        RC_40126 = 105,
        RC_40136 = 106,
        RC_40203 = 107,
        RC_40204 = 108,
        RC_40207 = 109,
        RC_40304 = 110,
        RC_40305 = 111,
        RC_40306 = 112,
        RC_40307 = 113,
        RC_50002 = 114,
        RC_50003 = 115,
        RC_50004 = 116,
        RC_50005 = 117,
        RC_50015 = 118,
        RC_50026 = 119,
        RC_50033 = 121,
        RC_50034 = 122,
        RC_50035 = 123,
        RC_50036 = 124,
        RC_50047 = 120,
        RC_50102 = 125,
        RC_50103 = 126,
        RC_50104 = 127,
        RC_50105 = 128,
        RC_50115 = 129,
        RC_50126 = 130,
        RC_50133 = 132,
        RC_50134 = 133,
        RC_50135 = 134,
        RC_50136 = 135,
        RC_50147 = 131,
        RC_50202 = 136,
        RC_50203 = 137,
        RC_50204 = 138,
        RC_50215 = 139,
        RC_50226 = 140,
        RC_50237 = 141,
        RC_50303 = 142,
        RC_50305 = 143,
        RC_60001 = 144,
        RC_60002 = 145,
        RC_60005 = 147,
        RC_60007 = 146,
        RC_60012 = 148,
        RC_60013 = 149,
        RC_60016 = 150,
        RC_60102 = 151,
        RC_60103 = 152,
        RC_60106 = 153,
        RC_60107 = 154,
        RC_60114 = 155,
        RC_60117 = 156,
        RC_60203 = 157,
        RC_60206 = 158,
        RC_60303 = 159,
        RC_60306 = 160,
        RC_60309 = 205,
        RC_60315 = 161,
        RC_60317 = 162,
        RC_60325 = 163,
        RC_60327 = 164,
        RC_60329 = 206,
        RC_60404 = 165,
        RC_60405 = 166,
        RC_60415 = 167,
        RC_60416 = 168,
        RC_70002 = 169,
        RC_70003 = 170,
        RC_70009 = 207,
        RC_70015 = 171,
        RC_70103 = 172,
        RC_70104 = 173,
        RC_70114 = 174,
        RC_70115 = 175,
        RC_70125 = 208,
        RC_70203 = 176,
        RC_70204 = 177,
        RC_80002 = 178,
        RC_80004 = 179,
        RC_80006 = 180,
        RC_80103 = 181,
        RC_80104 = 182,
        RC_80105 = 183,
        RC_80116 = 184,
        RC_80119 = 209,
        RC_80124 = 185,
        RC_80125 = 186,
        RC_80126 = 187,
        RC_80135 = 210,
        RC_80203 = 188,
        RC_80204 = 189,
        RC_80205 = 190,
        RC_80206 = 191,
        RC_80209 = 211,
        RC_80303 = 192,
        RC_80304 = 193,
        RC_80305 = 194,
        RC_80306 = 195,
        RC_BossQuiet_sr_010 = 216,
        RC_EX_gl_000 = 229,
        RC_EX_hg_000 = 228,
        RC_EX_hg_010 = 231,
        RC_EX_sr_000 = 230,
        RC_HoneyBee = 227,
        RC_None = 0,
        RC_Pr_ar_010 = 218,
        RC_Pr_sg_010 = 219,
        RC_Pr_sm_010 = 217,
        RC_Pr_sr_010 = 220,
        RC_Quiet_sr_010 = 213,
        RC_Quiet_sr_020 = 214,
        RC_Quiet_sr_030 = 215,
        RC_SP_hg_010 = 223,
        RC_SP_hg_020 = 224,
        RC_SP_sg_010 = 226,
        RC_SP_sm_010 = 225,
        RC_SkullFace_hg_010 = 222,
        RC_Volgin_sg_010 = 212,
        RC_mgm0_mgun0 = 221,
        RC_no_use_00 = 60,
    },
    barrelIDs = {
        BA_10001 = 1,
        BA_10004 = 2,
        BA_10024 = 3,
        BA_10101 = 4,
        BA_10102 = 5,
        BA_10201 = 6,
        BA_10302 = 7,
        BA_10403 = 8,
        BA_10405 = 9,
        BA_10407 = 10,
        BA_10414 = 11,
        BA_10415 = 12,
        BA_10417 = 13,
        BA_10424 = 14,
        BA_10425 = 15,
        BA_10503 = 16,
        BA_10504 = 17,
        BA_10603 = 18,
        BA_10703 = 19,
        BA_10704 = 20,
        BA_10705 = 21,
        BA_20002 = 22,
        BA_20103 = 23,
        BA_20203 = 24,
        BA_20215 = 25,
        BA_20225 = 26,
        BA_30001 = 28,
        BA_30003 = 29,
        BA_30023 = 30,
        BA_30024 = 31,
        BA_30035 = 32,
        BA_30036 = 33,
        BA_30043 = 34,
        BA_30044 = 35,
        BA_30102 = 36,
        BA_30103 = 37,
        BA_30107 = 38,
        BA_30113 = 39,
        BA_30115 = 40,
        BA_30123 = 41,
        BA_30124 = 42,
        BA_30201 = 43,
        BA_30203 = 44,
        BA_30213 = 45,
        BA_30214 = 46,
        BA_30223 = 47,
        BA_30224 = 48,
        BA_30303 = 49,
        BA_30304 = 50,
        BA_30314 = 51,
        BA_30315 = 52,
        BA_30325 = 53,
        BA_30326 = 54,
        BA_30334 = 55,
        BA_30335 = 56,
        BA_40001 = 57,
        BA_40032 = 58,
        BA_40034 = 59,
        BA_40042 = 60,
        BA_40043 = 61,
        BA_40102 = 62,
        BA_40103 = 63,
        BA_40133 = 64,
        BA_40134 = 65,
        BA_40135 = 66,
        BA_40143 = 67,
        BA_40203 = 68,
        BA_40205 = 69,
        BA_40304 = 70,
        BA_50002 = 71,
        BA_50102 = 72,
        BA_50133 = 73,
        BA_50202 = 74,
        BA_50203 = 75,
        BA_50204 = 76,
        BA_50303 = 77,
        BA_50305 = 78,
        BA_60001 = 79,
        BA_60004 = 80,
        BA_60005 = 81,
        BA_60102 = 82,
        BA_60105 = 83,
        BA_60107 = 84,
        BA_60203 = 85,
        BA_60205 = 86,
        BA_60303 = 87,
        BA_60305 = 88,
        BA_60404 = 89,
        BA_60405 = 90,
        BA_60407 = 91,
        BA_60415 = 92,
        BA_60416 = 93,
        BA_60417 = 94,
        BA_70002 = 95,
        BA_70004 = 96,
        BA_70006 = 97,
        BA_70024 = 98,
        BA_70103 = 99,
        BA_70105 = 100,
        BA_70106 = 101,
        BA_70116 = 102,
        BA_70203 = 103,
        BA_70205 = 104,
        BA_70207 = 105,
        BA_None = 0,
        BA_Pr_ar_010 = 111,
        BA_Pr_sg_010 = 112,
        BA_Pr_sm_010 = 110,
        BA_Pr_sr_010 = 113,
        BA_Quiet_sr_010 = 107,
        BA_Quiet_sr_020 = 108,
        BA_Quiet_sr_030 = 109,
        BA_Volgin_sg_010 = 106,
        BA_mgm0_mgun00 = 114,
        BA_no_use_00 = 27,
    },
    ammoIDs = {
        AM_10001 = 1,
        AM_10003 = 2,
        AM_10015 = 3,
        AM_10015_0 = 4,
        AM_1003a = 149,
        AM_10101 = 5,
        AM_10103 = 6,
        AM_10125 = 7,
        AM_10125_0 = 8,
        AM_10134 = 9,
        AM_10134_0 = 10,
        AM_10201 = 11,
        AM_10203 = 12,
        AM_10214 = 13,
        AM_10214_0 = 14,
        AM_10302 = 15,
        AM_10303 = 16,
        AM_10305 = 17,
        AM_10403 = 18,
        AM_10404 = 19,
        AM_10405 = 20,
        AM_10407 = 21,
        AM_10503 = 22,
        AM_10515 = 23,
        AM_10526 = 24,
        AM_10603 = 25,
        AM_10615 = 26,
        AM_10626 = 27,
        AM_10637 = 28,
        AM_10703 = 29,
        AM_10704 = 30,
        AM_10705 = 31,
        AM_20002 = 32,
        AM_20003 = 33,
        AM_20005 = 34,
        AM_20103 = 35,
        AM_20104 = 36,
        AM_20105 = 37,
        AM_20106 = 38,
        AM_20116 = 39,
        AM_20116_0 = 40,
        AM_20116_1 = 41,
        AM_20203 = 42,
        AM_20206 = 43,
        AM_20302 = 44,
        AM_20303 = 45,
        AM_20304 = 46,
        AM_20305 = 47,
        AM_30001 = 48,
        AM_30003 = 49,
        AM_30014 = 50,
        AM_30034 = 51,
        AM_30043 = 52,
        AM_30047 = 53,
        AM_30054 = 54,
        AM_30055 = 55,
        AM_30102 = 56,
        AM_30103 = 57,
        AM_30123 = 58,
        AM_30125 = 59,
        AM_30201 = 60,
        AM_30203 = 61,
        AM_30223 = 62,
        AM_30225 = 63,
        AM_30232 = 64,
        AM_30303 = 65,
        AM_30305 = 66,
        AM_30306 = 67,
        AM_30325 = 68,
        AM_40001 = 69,
        AM_40004 = 70,
        AM_40012 = 71,
        AM_40015 = 72,
        AM_40023 = 73,
        AM_40102 = 74,
        AM_40105 = 75,
        AM_40115 = 76,
        AM_40123 = 148,
        AM_40126 = 77,
        AM_40133 = 78,
        AM_40135 = 79,
        AM_40136 = 80,
        AM_40143 = 81,
        AM_40203 = 82,
        AM_40204 = 83,
        AM_40206 = 84,
        AM_40304 = 85,
        AM_50002 = 86,
        AM_50015 = 87,
        AM_50026 = 88,
        AM_50033 = 90,
        AM_50035 = 91,
        AM_50047 = 89,
        AM_50102 = 92,
        AM_50115 = 93,
        AM_50126 = 94,
        AM_50136 = 96,
        AM_50147 = 95,
        AM_50202 = 97,
        AM_50215 = 98,
        AM_50226 = 99,
        AM_50237 = 100,
        AM_50303 = 101,
        AM_50304 = 102,
        AM_50306 = 103,
        AM_60001 = 104,
        AM_60007 = 105,
        AM_60013 = 106,
        AM_60102 = 107,
        AM_60107 = 108,
        AM_60114 = 109,
        AM_60203 = 110,
        AM_60303 = 111,
        AM_60315 = 112,
        AM_60325 = 113,
        AM_60404 = 114,
        AM_60406 = 115,
        AM_60415 = 116,
        AM_60417 = 117,
        AM_70002 = 118,
        AM_70003 = 119,
        AM_70005 = 120,
        AM_70103 = 121,
        AM_70104 = 122,
        AM_70105 = 123,
        AM_70114 = 124,
        AM_70115 = 125,
        AM_70116 = 126,
        AM_70125 = 150,
        AM_70126 = 151,
        AM_70127 = 152,
        AM_70203 = 127,
        AM_70204 = 128,
        AM_70205 = 129,
        AM_80002 = 130,
        AM_80004 = 131,
        AM_80006 = 132,
        AM_80103 = 133,
        AM_80104 = 134,
        AM_80105 = 135,
        AM_80116 = 136,
        AM_80119 = 153,
        AM_8011a = 154,
        AM_8011b = 155,
        AM_80124 = 137,
        AM_80125 = 138,
        AM_80126 = 139,
        AM_80135 = 156,
        AM_80136 = 157,
        AM_80138 = 158,
        AM_8013a = 159,
        AM_8013b = 160,
        AM_80203 = 140,
        AM_80204 = 141,
        AM_80205 = 142,
        AM_80206 = 143,
        AM_80303 = 144,
        AM_80304 = 145,
        AM_80305 = 146,
        AM_80306 = 147,
        AM_EX_gl_000 = 176,
        AM_EX_hg_000 = 175,
        AM_EX_hg_000_1 = 178,
        AM_EX_hg_000_2 = 179,
        AM_EX_hg_000_3 = 180,
        AM_EX_hg_000_4 = 181,
        AM_EX_hg_010 = 182,
        AM_EX_hg_011 = 183,
        AM_EX_hg_012 = 184,
        AM_EX_hg_013 = 185,
        AM_EX_sr_000 = 177,
        AM_None = 0,
        AM_Pr_ar_010 = 166,
        AM_Pr_sg_010 = 167,
        AM_Pr_sm_010 = 165,
        AM_Pr_sr_010 = 168,
        AM_Quiet_sr_010 = 162,
        AM_Quiet_sr_020 = 163,
        AM_Quiet_sr_030 = 164,
        AM_SP_hg_010 = 171,
        AM_SP_hg_020 = 172,
        AM_SP_sg_010 = 174,
        AM_SP_sm_010 = 173,
        AM_SkullFace_hg_010 = 170,
        AM_Volgin_sg_010 = 161,
        AM_mgm0_mgun0 = 169,
    },
    stockIDs = {
        SK_20002 = 1,
        SK_20015 = 2,
        SK_20103 = 3,
        SK_20106 = 4,
        SK_20203 = 5,
        SK_20216 = 6,
        SK_30001 = 9,
        SK_30002 = 10,
        SK_30024 = 11,
        SK_30043 = 12,
        SK_30045 = 13,
        SK_30102 = 14,
        SK_30113 = 15,
        SK_30123 = 16,
        SK_30201 = 17,
        SK_30205 = 18,
        SK_30213 = 19,
        SK_30223 = 20,
        SK_40001 = 21,
        SK_40043 = 22,
        SK_40102 = 23,
        SK_40144 = 24,
        SK_40203 = 25,
        SK_40306 = 26,
        SK_50002 = 27,
        SK_50004 = 28,
        SK_50102 = 29,
        SK_50202 = 30,
        SK_50303 = 31,
        SK_60001 = 32,
        SK_60002 = 33,
        SK_60102 = 34,
        SK_60103 = 35,
        SK_60203 = 36,
        SK_60205 = 37,
        SK_60303 = 38,
        SK_60304 = 39,
        SK_60405 = 40,
        SK_70103 = 41,
        SK_70203 = 42,
        SK_None = 0,
        SK_no_use_00 = 7,
        SK_no_use_01 = 8,
    },
    mzIDs = {
        MZ_30001 = 1,
        MZ_30023 = 2,
        MZ_30102 = 3,
        MZ_30105 = 4,
        MZ_30123 = 5,
        MZ_30201 = 6,
        MZ_30213 = 7,
        MZ_30223 = 8,
        MZ_30232 = 9,
        MZ_30303 = 10,
        MZ_40102 = 11,
        MZ_40203 = 12,
        MZ_40206 = 13,
        MZ_40304 = 14,
        MZ_60001 = 15,
        MZ_60102 = 16,
        MZ_60105 = 17,
        MZ_60203 = 18,
        MZ_60303 = 19,
        MZ_60305 = 20,
        MZ_60404 = 21,
        MZ_60405 = 22,
        MZ_60416 = 23,
        MZ_70002 = 24,
        MZ_70103 = 25,
        MZ_70106 = 26,
        MZ_70203 = 27,
        MZ_70207 = 28,
        MZ_None = 0,
    },
    moIDs = {
        MO_10002 = 3,
        MO_10004 = 4,
        MO_10026 = 5,
        MO_10101 = 1,
        MO_10105 = 2,
        MO_10202 = 6,
        MO_10205 = 7,
        MO_10306 = 8,
        MO_10407 = 9,
        MO_10425 = 10,
        MO_20002 = 11,
        MO_20005 = 12,
        MO_20006 = 13,
        MO_20204 = 14,
        MO_20205 = 15,
        MO_20206 = 16,
        MO_20302 = 17,
        MO_20307 = 18,
        MO_30002 = 20,
        MO_30005 = 21,
        MO_30025 = 23,
        MO_30046 = 22,
        MO_30102 = 24,
        MO_30105 = 25,
        MO_30115 = 26,
        MO_30117 = 19,
        MO_30205 = 27,
        MO_30235 = 28,
        MO_30237 = 38,
        MO_40104 = 29,
        MO_40106 = 30,
        MO_40306 = 31,
        MO_60103 = 32,
        MO_60106 = 33,
        MO_60204 = 34,
        MO_60206 = 35,
        MO_60406 = 36,
        MO_6040a = 39,
        MO_70025 = 37,
        MO_None = 0,
    },
    stIDs = {
        ST_20104 = 1,
        ST_20205 = 2,
        ST_30114 = 3,
        ST_30202 = 4,
        ST_30204 = 5,
        ST_30303 = 6,
        ST_30305 = 7,
        ST_30306 = 8,
        ST_50003 = 9,
        ST_50133 = 10,
        ST_50136 = 11,
        ST_50303 = 12,
        ST_60001 = 13,
        ST_60004 = 14,
        ST_60102 = 15,
        ST_60105 = 16,
        ST_60203 = 17,
        ST_60303 = 18,
        ST_60304 = 19,
        ST_60407 = 20,
        ST_80002 = 21,
        ST_80103 = 22,
        ST_80203 = 23,
        ST_80303 = 24,
        ST_None = 0,
    },
    ubIDs = {
        UB_10006 = 5,
        UB_10107 = 6,
        UB_10207 = 7,
        UB_20105 = 1,
        UB_30005 = 2,
        UB_30105 = 3,
        UB_30304 = 4,
        UB_40043 = 8,
        UB_40045 = 9,
        UB_40144 = 10,
        UB_50002 = 11,
        UB_50015 = 12,
        UB_50026 = 13,
        UB_50033 = 15,
        UB_50035 = 16,
        UB_50047 = 14,
        UB_50102 = 17,
        UB_50115 = 18,
        UB_50126 = 19,
        UB_50133 = 21,
        UB_50136 = 22,
        UB_50147 = 20,
        UB_None = 0,
    },
    ltsIDs = {
        LS_10415 = 6,
        LS_20004 = 7,
        LS_30104 = 8,
        LS_40034 = 9,
        LT_10102 = 1,
        LT_10104 = 2,
        LT_30025 = 3,
        LT_30105 = 4,
        LT_40103 = 5,
        LT_None = 0,
    },
    barrelLengthIds = {
        BARREL_LENGTH_LONG = 2,
        BARREL_LENGTH_MIDDLE = 1,
        BARREL_LENGTH_SHORT = 0,
    },
    blIDs = {
        BL_10mmAuto = 6,
        BL_12GA00Back = 18,
        BL_12GA00BackMag = 19,
        BL_12GAAir = 22,
        BL_12GAAirW = 101,
        BL_12GASrug = 20,
        BL_12GASrugP = 21,
        BL_20mmGrenade = 31,
        BL_20mmRocket = 35,
        BL_20mmSleep = 34,
        BL_20mmSmoke = 32,
        BL_20mmStun = 33,
        BL_40mmGrenade = 27,
        BL_40mmSleep = 30,
        BL_40mmSmoke = 28,
        BL_40mmStun = 29,
        BL_Cannon = 1,
        BL_EX_gl_000 = 103,
        BL_EX_hg_000 = 102,
        BL_EX_hg_010 = 106,
        BL_EX_sr_000 = 104,
        BL_Flare = 69,
        BL_HAND_Jehuty = 243,
        BL_HAND_KillRocket = 245,
        BL_HAND_StunArm = 242,
        BL_HAND_StunRocket = 244,
        BL_HeliChainGun = 86,
        BL_Hg114Auto = 2,
        BL_Hg114AutoP = 3,
        BL_Hg114AutoPP = 107,
        BL_Hg44Mag = 8,
        BL_Hg44MagP = 9,
        BL_Hg765x21mm = 4,
        BL_Hg9x19mm = 5,
        BL_HgGrenade = 23,
        BL_HgRubber = 7,
        BL_HgSleep = 26,
        BL_HgSmoke = 24,
        BL_HgStun = 25,
        BL_HgTranq = 10,
        BL_HgTranqP = 11,
        BL_Mortar = 64,
        BL_Nad0 = 63,
        BL_None = 0,
        BL_ParasiteHardRicochet = 100,
        BL_Rf556mmMachinegun = 41,
        BL_Rf556mmSniperMag = 36,
        BL_Rf556x45mm = 15,
        BL_Rf762mmMachinegun = 42,
        BL_Rf762mmSniperMag = 37,
        BL_Rf762mmSniperMagAP = 39,
        BL_Rf762mmSniperMagP = 38,
        BL_Rf762x51mm = 16,
        BL_RfRubber = 17,
        BL_RfTranq = 40,
        BL_SupplyBomb = 65,
        BL_SupplyChaff = 68,
        BL_SupplySleep = 67,
        BL_SupplySmoke = 66,
        BL_TankCannon = 72,
        BL_TankCannonHoming = 74,
        BL_Tankgun_105mmRifledBoreGun = 78,
        BL_Tankgun_105mmRifledBoreGun_Homing = 81,
        BL_Tankgun_120mmSmoothBoreGun = 79,
        BL_Tankgun_120mmSmoothBoreGun_Homing = 82,
        BL_Tankgun_125mmSmoothBoreGun = 80,
        BL_Tankgun_125mmSmoothBoreGun_Homing = 83,
        BL_Tankgun_20mmAutoCannon = 76,
        BL_Tankgun_30mmAutoCannon = 77,
        BL_Tankgun_82mmRocketPoweredProjectile = 84,
        BL_Tankgun_MultipleRocketLauncher = 85,
        BL_Turret = 62,
        BL_UavGrenade = 96,
        BL_UavLMG = 99,
        BL_UavSleepGasGrenade = 98,
        BL_UavSmokeGrenade = 97,
        BL_VolBullet = 61,
        BL_Water = 12,
        BL_WaterP = 13,
        BL_WaterPP = 14,
        BL_Wav1 = 70,
        BL_WavCannon = 71,
        BL_WavCannonHoming = 73,
        BL_WavRocket = 75,
        BL_mgm0_ammo0 = 90,
        BL_mgm0_famo0 = 91,
        BL_mgm0_mgun0 = 89,
        BL_mgs0_grnd0 = 95,
        BL_mgs0_miss0 = 93,
        BL_mgs0_miss1 = 92,
        BL_mgs0_srcm0 = 94,
        BL_ms00 = 43,
        BL_ms00_G2 = 44,
        BL_ms00_G3 = 45,
        BL_ms01 = 57,
        BL_ms01_G2 = 58,
        BL_ms01_G3 = 59,
        BL_ms01_G4 = 60,
        BL_ms02 = 46,
        BL_ms02F = 50,
        BL_ms02F_G2 = 51,
        BL_ms02F_G3 = 52,
        BL_ms02Fulton = 105,
        BL_ms02Sleep = 49,
        BL_ms02_G2 = 47,
        BL_ms02_G3 = 48,
        BL_ms03 = 53,
        BL_ms03_G2 = 54,
        BL_ms03_G3 = 55,
        BL_ms03_G4 = 56,
        BL_uth0_ammo0 = 87,
        BL_uth0_ammo1 = 88,
    },
    blaIDs = {
        BLA_20mmGrenade = 0,
        BLA_20mmGrenadeS = 1,
        BLA_AntitankMine = 16,
        BLA_BlastArm = 25,
        BLA_BombAssist = 45,
        BLA_C4 = 11,
        BLA_Cannon = 26,
        BLA_ChaffAssist = 48,
        BLA_Claymore = 15,
        BLA_Com_ms_010 = 10,
        BLA_DMine = 15,
        BLA_Drum = 42,
        BLA_East_ms_010 = 8,
        BLA_ElectromagneticNetMine = 23,
        BLA_FlareBomb = 40,
        BLA_GasTank = 43,
        BLA_GimmickCommon = 41,
        BLA_Grenade = 12,
        BLA_HydrantTank = 44,
        BLA_MolotovCocktail = 21,
        BLA_MolotovCocktailPlaced = 22,
        BLA_Mortar = 39,
        BLA_SleepGusAssist = 46,
        BLA_SleepingGusGrenade = 20,
        BLA_SleepingGusMine = 24,
        BLA_SmokeAssist = 47,
        BLA_SmokeGrenade = 14,
        BLA_StunGrenade = 19,
        BLA_SupplyFlareGrenade = 18,
        BLA_SupportHeliFlareGrenade = 17,
        BLA_TankCannon = 29,
        BLA_WarningFlare = 13,
        BLA_Wav1 = 27,
        BLA_WavCannon = 28,
        BLA_WavRocket = 30,
        BLA_West_ms_010 = 9,
        BLA_mgm0_ammo0 = 33,
        BLA_mgm0_famo0 = 34,
        BLA_mgs0_grnd0 = 38,
        BLA_mgs0_miss0 = 36,
        BLA_mgs0_miss1 = 35,
        BLA_mgs0_srcm0 = 37,
        BLA_ms00 = 2,
        BLA_ms02 = 3,
        BLA_ms02F = 4,
        BLA_ms02F_G2 = 5,
        BLA_ms02F_G3 = 6,
        BLA_ms03 = 7,
        BLA_uth0_ammo0 = 31,
        BLA_uth0_ammo1 = 32,
    },
    scopeUIIds = {
        SCOPE_UI_DEFAULT = 1,
        SCOPE_UI_DOT = 2,
        SCOPE_UI_NONE = 0,
    },
    eqpAmmoIDs = {
        EQP_AM_10001 = 141,
        EQP_AM_10003 = 142,
        EQP_AM_10015 = 143,
        EQP_AM_1003a = 339,
        EQP_AM_10101 = 144,
        EQP_AM_10103 = 145,
        EQP_AM_10125 = 146,
        EQP_AM_10134 = 147,
        EQP_AM_10201 = 148,
        EQP_AM_10203 = 149,
        EQP_AM_10214 = 150,
        EQP_AM_10302 = 151,
        EQP_AM_10303 = 152,
        EQP_AM_10305 = 153,
        EQP_AM_10403 = 154,
        EQP_AM_10404 = 155,
        EQP_AM_10405 = 156,
        EQP_AM_10407 = 157,
        EQP_AM_10503 = 158,
        EQP_AM_10515 = 159,
        EQP_AM_10526 = 160,
        EQP_AM_20002 = 161,
        EQP_AM_20003 = 162,
        EQP_AM_20005 = 163,
        EQP_AM_20103 = 164,
        EQP_AM_20104 = 165,
        EQP_AM_20105 = 166,
        EQP_AM_20106 = 167,
        EQP_AM_20116 = 168,
        EQP_AM_20203 = 169,
        EQP_AM_20206 = 170,
        EQP_AM_20302 = 171,
        EQP_AM_20303 = 172,
        EQP_AM_20304 = 173,
        EQP_AM_20305 = 174,
        EQP_AM_30001 = 175,
        EQP_AM_30003 = 176,
        EQP_AM_30014 = 177,
        EQP_AM_30034 = 178,
        EQP_AM_30043 = 179,
        EQP_AM_30047 = 180,
        EQP_AM_30054 = 181,
        EQP_AM_30055 = 182,
        EQP_AM_30102 = 183,
        EQP_AM_30103 = 184,
        EQP_AM_30123 = 185,
        EQP_AM_30125 = 186,
        EQP_AM_30201 = 187,
        EQP_AM_30203 = 188,
        EQP_AM_30223 = 189,
        EQP_AM_30225 = 190,
        EQP_AM_30232 = 191,
        EQP_AM_30303 = 192,
        EQP_AM_30305 = 193,
        EQP_AM_30306 = 194,
        EQP_AM_30325 = 195,
        EQP_AM_40001 = 196,
        EQP_AM_40004 = 197,
        EQP_AM_40012 = 198,
        EQP_AM_40015 = 199,
        EQP_AM_40023 = 200,
        EQP_AM_40102 = 201,
        EQP_AM_40105 = 202,
        EQP_AM_40115 = 203,
        EQP_AM_40123 = 334,
        EQP_AM_40126 = 204,
        EQP_AM_40133 = 205,
        EQP_AM_40135 = 206,
        EQP_AM_40136 = 207,
        EQP_AM_40143 = 208,
        EQP_AM_40203 = 209,
        EQP_AM_40204 = 210,
        EQP_AM_40206 = 211,
        EQP_AM_40304 = 212,
        EQP_AM_50102 = 213,
        EQP_AM_50115 = 214,
        EQP_AM_50126 = 215,
        EQP_AM_50136 = 217,
        EQP_AM_50147 = 216,
        EQP_AM_50202 = 218,
        EQP_AM_50215 = 219,
        EQP_AM_50226 = 220,
        EQP_AM_50237 = 221,
        EQP_AM_50303 = 222,
        EQP_AM_50304 = 223,
        EQP_AM_50306 = 224,
        EQP_AM_60001 = 225,
        EQP_AM_60007 = 226,
        EQP_AM_60013 = 227,
        EQP_AM_60102 = 228,
        EQP_AM_60107 = 229,
        EQP_AM_60114 = 230,
        EQP_AM_60203 = 231,
        EQP_AM_60303 = 232,
        EQP_AM_60315 = 233,
        EQP_AM_60325 = 234,
        EQP_AM_60404 = 235,
        EQP_AM_60406 = 236,
        EQP_AM_60415 = 237,
        EQP_AM_60417 = 238,
        EQP_AM_70002 = 239,
        EQP_AM_70003 = 240,
        EQP_AM_70005 = 241,
        EQP_AM_70103 = 242,
        EQP_AM_70104 = 243,
        EQP_AM_70105 = 244,
        EQP_AM_70114 = 245,
        EQP_AM_70115 = 246,
        EQP_AM_70116 = 247,
        EQP_AM_70125 = 340,
        EQP_AM_70126 = 341,
        EQP_AM_70127 = 342,
        EQP_AM_70203 = 248,
        EQP_AM_70204 = 249,
        EQP_AM_70205 = 250,
        EQP_AM_EX_gl_000 = 336,
        EQP_AM_EX_hg_000 = 335,
        EQP_AM_EX_hg_010 = 344,
        EQP_AM_EX_sr_000 = 338,
        EQP_AM_Pr_ar_010 = 255,
        EQP_AM_Pr_sg_010 = 256,
        EQP_AM_Pr_sm_010 = 254,
        EQP_AM_Pr_sr_010 = 257,
        EQP_AM_Quiet_sr_010 = 251,
        EQP_AM_Quiet_sr_020 = 252,
        EQP_AM_Quiet_sr_030 = 253,
        EQP_AM_SP_hg_010 = 260,
        EQP_AM_SP_hg_020 = 261,
        EQP_AM_SP_sg_010 = 263,
        EQP_AM_SP_sm_010 = 262,
        EQP_AM_SkullFace_hg_010 = 259,
        EQP_AM_Volgin_sg_010 = 258,
    },
    penetrateIDs = {
        PENETRATE_LEVEL_AMRIFLE = 5,
        PENETRATE_LEVEL_HANDGUN = 2,
        PENETRATE_LEVEL_MINIMUM = 0,
        PENETRATE_LEVEL_RIFLE = 3,
        PENETRATE_LEVEL_SNIPER = 4,
        PENETRATE_LEVEL_TRANQ = 1,
    },
    richocetIDs = {
        RICOCHET_SIZE_DEFAULT = 0,
        RICOCHET_SIZE_LARGE = 1,
        RICOCHET_SIZE_SMALL = 2,
    },
    bulletTypeIDs = {
        BULLET_TYPE_AIRSHOCK = 5,
        BULLET_TYPE_BLAST = 2,
        BULLET_TYPE_NORMAL = 0,
        BULLET_TYPE_SHELL = 3,
        BULLET_TYPE_SPREAD = 1,
        BULLET_TYPE_WATER = 4,
    },
}

--UI
function this.ModMenu(menu)
	ZetaMenu.CreateModMenu(this,menu)

    --Receiver 
    this.CreateEditorOption(this,menu,{
        title = "Receiver Parameters",
        description = "Modify receiver parameters",
        menuName =  "Receiver",
        labels = this.receiverLabels,
        func = function() return this.GetReceiverParams() end,
        tables = {
            [1] = this.recvIDs,
            [2] = this.atkIDs,
        },
    })
    ZetaMenu.AddModItemToMenu(this, menu,
    "PrintReceiverParameters",  
    "PrintReceiverParameters", 
    "Print Receiver Parameters", 
    "Prints Current Weapon Receiver Parameters.",
    "Receiver")
    this.CreateEditorOption(this,menu,{
        title = "Receiver Set Base Parameters",
        description = "Modify receiver set base parameters",
        menuName =  "IronSight",
        labels = this.receiverSetBaseLabels,
        func = function() return this.GetReceiverParams(3, ZetaEquipParameters.equipParameters.receiverParamSetsBase) end,
    })
    this.CreateEditorOption(this,menu,{
        title = "Receiver Set Wobbling Parameters",
        description = "Modify receiver set wobbling parameters",
        menuName =  "Wobbling",
        labels = this.receiverWobblingLabels,
        func = function() return this.GetReceiverParams(4, ZetaEquipParameters.equipParameters.receiverParamSetsWobbling) end,
    })
    this.CreateEditorOption(this,menu,{
        title = "Receiver Set Systems Parameters",
        description = "Modify receiver set systems parameters",
        menuName =  "Systems",
        labels = this.receiverSystemLabels,
        func = function() return this.GetReceiverParams(5, ZetaEquipParameters.equipParameters.receiverParamSetsSystem) end,
        tables = {
            [1] = this.eqpTypes,
            [2] = this.reticleTypes,
            [3] = this.triggerTypes,
        },
    })
    --Barrel
    this.CreateEditorOption(this,menu,{
        title = "Barrel Parameters",
        description = "Modify barrel parameters",
        menuName =  "Barrel",
        labels = this.barrelLabels,
        func = function() return this.GetBarrelParams() end,
        tables = {
            [1] = this.barrelIDs,
            [3] = this.barrelLengthIds,
        },
    })
    this.CreateEditorOption(this,menu,{
        title = "Barrel Set Base Parameters",
        description = "Modify barrel set base parameters",
        menuName =  "BarrelSetBase",
        labels = this.barrelSetBaseParamLabels,
        func = function() return this.GetBarrelParams(2,ZetaEquipParameters.equipParameters.barrelParamSetsBase) end,
    })
    --General
    this.CreateEditorOption(this,menu,{
        title = "Attack Parameters",
        description = "Modify weapon attack parameters",
        menuName =  "DamageParams",
        labels = this.damageLabels,
        func = function() return this.GetDamageParameters() end,
        tables = {
            [1] = this.atkIDs,
            [9] = this.injTypes,
            [10] = this.injParts,
            [29] = this.damSources,
        },
    })
    this.CreateEditorOption(this,menu,{
        title = "GunBasic Parameters",
        description = "Modify weapon gunBasic parameters",
        menuName =  "GunBasicParams",
        labels = this.gunBasicLabels,
        func = function() return this.GetGunBasicParams() end,
        tables = {
            [1] = this.wpIDs,
            [2] = this.recvIDs,
            [3] = this.barrelIDs,
            [4] = this.ammoIDs,
            [5] = this.stockIDs,
            [6] = this.mzIDs,
            [7] = this.moIDs,
            [8] = this.stIDs,
            [9] = this.stIDs,
            [10] = this.ubIDs,
            [11] = this.ltsIDs,
            [12] = this.ltsIDs,
        },
    })

    this.CreateEditorOption(this,menu,{
        title = "Magazine Parameters",
        description = "Modify Magazine parameters",
        menuName =  "Magazine",
        labels = this.magazineLabels,
        func = function() return this.GetMagazineParams() end,
        tables = {
            [1] = this.ammoIDs,
            [2] = this.eqpAmmoIDs,
            [5] = this.blIDs,
        },
    })

    this.CreateEditorOption(this,menu,{
        title = "Stock Parameters",
        description = "Modify stock parameters",
        menuName =  "Stock",
        labels = this.stockLabels,
        func = function() return this.GetStockParams() end,
        tables = {
            [1] = this.stockIDs,
        },
    })

    this.CreateEditorOption(this,menu,{
        title = "Muzzle Options Parameters",
        description = "Modify muzzle options parameters",
        menuName =  "MuzzleOp",
        labels = this.muzzleOpLabels,
        func = function() return this.GetMuzzleOptionParams() end,
        tables = {
            [1] = this.moIDs,
        },
    })

    this.CreateEditorOption(this,menu,{
        title = "Sight 1 Parameters",
        description = "Modify sight 1 parameters",
        menuName = "Sight",
        labels = this.sightScopeLabels,
        func = function() return this.GetSightParams() end,
        tables = {
            [1] = this.stIDs,
            [5] = this.scopeUIIds,
        },
    })

    this.CreateEditorOption(this,menu,{
        title = "Sight 2 Parameters",
        description = "Modify sight 2 parameters",
        menuName =  "SightSec",
        labels = this.sightScopeLabels,
        func = function() return this.GetSightParams(true) end,
        tables = {
            [1] = this.stIDs,
            [5] = this.scopeUIIds,
        },
    })

    this.CreateEditorOption(this,menu,{
        title = "Underbarrel Parameters",
        description = "Modify Underbarrel parameters",
        menuName =  "Underbarrel",
        labels = this.underBarrelLabels,
        func = function() return this.GetUnderBarrelParams() end,
        tables = {
            [1] = this.ubIDs,
            [2] = this.recvIDs,
            [3] = this.ammoIDs,
        },
    })

    this.CreateEditorOption(this,menu,{
        title = "Bullet Parameters",
        description = "Modify bullet parameters",
        menuName =  "Bullet",
        labels = this.bulletLabels,
        func = function() return this.GetBulletParams() end,
        tables = {
            [1] = this.blIDs,
            [9] = this.richocetIDs,
            [10] = this.bulletTypeIDs,
            [11] = this.blaIDs,
            [13] = this.eqpTypes,
        },
    })

    this.CreateEditorOption(this,menu,{
        title = "Bullet Set Base 1 Parameters",
        description = "Modify bullet Set Base 2 parameters",
        menuName = "BulletSetBase",
        labels = this.barrelSetBaseParamLabels,
        func = function() return this.GetBulletSetBaseParams(5, ZetaEquipParameters.equipParameters.bulletParamSetsBase) end,
        tables = {
            [10] = this.penetrateIDs,
            [11] = this.penetrateIDs,
        },
    })
    this.CreateEditorOption(this,menu,{
        title = "Bullet Set Base 2 Parameters",
        description = "Modify bullet Set Base 2 parameters",
        menuName =  "BulletSetBaseSec",
        labels = this.barrelSetBaseParamLabels,
        func = function() return this.GetBulletSetBaseParams(6, ZetaEquipParameters.equipParameters.bulletParamSetsBase) end,
        tables = {
            [10] = this.penetrateIDs,
            [11] = this.penetrateIDs,
        },
    })
end

function this.CreateEditorOption(this,menu,info)
    ZetaMenu.CreateModMenu(this, menu, info.title, info.description, info.menuName)
    for i, value in ipairs(info.labels) do
        local menuFunc = this.CustomOption(-8192,8192,0.01,0,function(self) 
            local sysParam = info.func()[i]
            if sysParam == nil then sysParam = 0 end
            self:Set(sysParam) 
        end )
        if info.tables ~= nil and next(info.tables) then
            local altTable = info.tables[i]
            if altTable ~= nil and next(altTable) then
                menuFunc = this.CustomListOption( altTable,0,function(self)
                    local sysParam = info.func()[i]
                    if sysParam == nil then sysParam = 0 end
                    self:Set(sysParam) 
                end )
            end
        end
        ZetaMenu.AddModItemToMenu(this, menu, menuFunc, info.menuName.."P"..i, value, "Changes value for "..value, info.menuName)
    end
end
function this.CustomOption(itemMin,itemMax,itemInc,itemDefault,onSelect)
	local ret = {
		inMission=true,
		range={min=itemMin,max=itemMax,increment=itemInc},
		default=itemDefault,
        OnChange=function()end,
		OnSelect=onSelect,
	} 
	return ret
end
function this.CustomListOption(tableToList,itemDefault,onSelect)
    local newSettings = {}
    for key, value in pairs(tableToList) do table.insert(newSettings, key) end
	local ret = {
		inMission=true,
		settings=newSettings,
		default=itemDefault,
        OnChange=function()end,
        GetSettingText=function(self,setting)
            for key, value in pairs(tableToList) do
                if setting == value then return key end
            end
            return ""
        end,
        OnSelect=onSelect,
	} 
	return ret
end

--Get Weapon
function this.GetPlayerWeapon()
    local weaponEqp = ZetaPlayer.GetHeldEquip()
    if weaponEqp ~= nil then
        for key, value in pairs(this.equipIDs) do
            if weaponEqp == value then 
                local newKey = string.gsub( key, "EQP_", "")
                return this.wpIDs[newKey]
            end
        end
    end
    return nil
end

function this.GetGunBasicParams()
    local playerEquip = this.GetPlayerWeapon()
    if playerEquip ~= nil then
        local gunBasic = this.GetGunBasicEntry( playerEquip )
        if gunBasic ~= nil then 
            return gunBasic 
        end
    end
    return nil
end

--Table Funcs
function this.GetBarrelParams(index, table)
    local barrel = this.GetBarrelEntry( this.GetGunBasicParams() )
    if barrel ~= nil then
        if index ~= nil then
            if table ~= nil and next(table) then
                local setBase = table[barrel[index]+1] 
                if setBase ~= nil then return setBase end
            end
        end
        return barrel
    end
    return nil
end

function this.GetReceiverParams(index, table)
    local receiver = this.GetReceiverEntry( this.GetGunBasicParams() )
    if receiver ~= nil then
        if index ~= nil then
            if table ~= nil and next(table) then
                local setBase = table[receiver[index]+1] 
                if setBase ~= nil then return setBase end
            end
        end
        return receiver
    end
    return nil
end

function this.GetBulletSetBaseParams(index, table)
    local bullet = this.GetBulletParams()
    if bullet ~= nil then
        if index ~= nil then
            if table ~= nil and next(table) then
                local setBase = table[bullet[index]+1] 
                if setBase ~= nil then return setBase end
            end
        end
        return bullet
    end
    return nil
end

function this.GetMagazineParams()
    local mag = this.GetMagazineEntry( this.GetGunBasicParams() )
    if mag ~= nil then  return mag end
    return nil
end

function this.GetStockParams()
    local stock = this.GetStockEntry( this.GetGunBasicParams() )
    if stock ~= nil then return stock end
    return nil
end

function this.GetUnderBarrelParams()
    local underBarrel = this.GetUnderBarrelEntry( this.GetGunBasicParams() )
    if underBarrel ~= nil then return underBarrel end
    return nil
end

function this.GetBulletParams()
    local bullet = this.GetBulletEntry( this.GetMagazineParams() )
    if bullet ~= nil then return bullet end
    return nil
end

function this.GetSightParams(sec)
    local sight = this.GetSightEntry( this.GetGunBasicParams(), sec )
    if sight ~= nil then return sight end
    return nil
end

function this.GetMuzzleOptionParams()
    local muzOp = this.GetMuzzleOptionEntry( this.GetGunBasicParams() )
    if muzOp ~= nil then return muzOp end
    return nil
end

function this.GetDamageParameters()
    local receiver = this.GetReceiverEntry( this.GetGunBasicParams() )
    if receiver ~= nil then
        local atkID = this.GetDamageEntry(receiver[2])
        if atkID ~= nil then return atkID end
    end
    return nil
end

--Single Entry Functions
function this.GetWeaponEntry(equip)
    if equip ~= nil then
        local constTable = ZetaEquipIdTable.equipIdTable
        if constTable ~= nil and next(constTable)then
            local indexOfEqp = ZetaUtil.GetIndex(equip,constTable)
            if indexOfEqp ~= nil then
                return constTable[indexOfEqp][3]
            end
        end
    end
    return nil
end
function this.GetGunBasicEntry(weapon)
    if weapon ~= nil then
        local gunBasicTable = ZetaEquipParameters.equipParameters.gunBasic
        if gunBasicTable ~= nil and next(gunBasicTable)then
            local indexOfWp = ZetaUtil.GetIndex(weapon,gunBasicTable)
            if indexOfWp ~= nil then
                return gunBasicTable[indexOfWp]
            end
        end
    end
    return nil
end
function this.GetReceiverEntry(gunBasic)
    if gunBasic ~= nil then
        local receiverTable = ZetaEquipParameters.equipParameters.receiver
        if receiverTable ~= nil and next(receiverTable)then
            local indexOfRecv = ZetaUtil.GetIndex(gunBasic[2],receiverTable)
            if indexOfRecv ~= nil then
                return receiverTable[indexOfRecv]
            end
        end
    end
    return nil
end
function this.GetMuzzleOptionEntry(gunBasic)
    if gunBasic ~= nil then
        local muzzleOpTable = ZetaEquipParameters.equipParameters.muzzleOption
        if muzzleOpTable ~= nil and next(muzzleOpTable)then
            local indexOfRecv = ZetaUtil.GetIndex(gunBasic[7],muzzleOpTable)
            if indexOfRecv ~= nil then
                return muzzleOpTable[indexOfRecv]
            end
        end
    end
    return nil
end
function this.GetSightEntry(gunBasic,sec)
    if gunBasic ~= nil then
        local sightTable = ZetaEquipParameters.equipParameters.sight
        if sightTable ~= nil and next(sightTable)then
            local indexToUse = 8
            if sec == true then indexToUse = 9 end
            local indexOfRecv = ZetaUtil.GetIndex(gunBasic[indexToUse],sightTable)
            if indexOfRecv ~= nil then
                return sightTable[indexOfRecv]
            end
        end
    end
    return nil
end
function this.GetStockEntry(gunBasic)
    if gunBasic ~= nil then
        local stockTable = ZetaEquipParameters.equipParameters.stock
        if stockTable ~= nil and next(stockTable)then
            local indexOfRecv = ZetaUtil.GetIndex(gunBasic[5],stockTable)
            if indexOfRecv ~= nil then
                return stockTable[indexOfRecv]
            end
        end
    end
    return nil
end
function this.GetMagazineEntry(gunBasic)
    if gunBasic ~= nil then
        local magazineTable = ZetaEquipParameters.equipParameters.magazine
        if magazineTable ~= nil and next(magazineTable)then
            local indexOfRecv = ZetaUtil.GetIndex(gunBasic[4],magazineTable)
            if indexOfRecv ~= nil then
                return magazineTable[indexOfRecv]
            end
        end
    end
    return nil
end
function this.GetBarrelEntry(gunBasic)
    if gunBasic ~= nil then
        local barrelTable = ZetaEquipParameters.equipParameters.barrel
        if barrelTable ~= nil and next(barrelTable)then
            local indexOfRecv = ZetaUtil.GetIndex(gunBasic[3],barrelTable)
            if indexOfRecv ~= nil then
                return barrelTable[indexOfRecv]
            end
        end
    end
    return nil
end
function this.GetUnderBarrelEntry(gunBasic)
    if gunBasic ~= nil then
        local underBarrelTable = ZetaEquipParameters.equipParameters.underBarrel
        if underBarrelTable ~= nil and next(underBarrelTable)then
            local indexOfRecv = ZetaUtil.GetIndex(gunBasic[10],underBarrelTable)
            if indexOfRecv ~= nil then
                return underBarrelTable[indexOfRecv]
            end
        end
    end
    return nil
end
function this.GetBulletEntry(magazine)
    if magazine ~= nil then
        local bulletTable = ZetaEquipParameters.equipParameters.bullet
        if bulletTable ~= nil and next(bulletTable)then
            local indexOfRecv = ZetaUtil.GetIndex(magazine[5],bulletTable)
            if indexOfRecv ~= nil then
                return bulletTable[indexOfRecv]
            end
        end
    end
    return nil
end
function this.GetDamageEntry(attackID)
    if attackID ~= nil then
        local damTable = ZetaDamageParameterTables.DamageParameterTable
        if damTable ~= nil and next(damTable)then
            local indexOfAtk = ZetaUtil.GetIndex(attackID,damTable)
            if indexOfAtk ~= nil then
                return damTable[indexOfAtk]
            end
        end
    end
    return nil
end

--Print Functions
function this.PrintReceiverParameters()
    local playerEquip = this.GetPlayerWeapon()
    if playerEquip ~= nil then
        local receiver = this.GetReceiverEntry( this.GetGunBasicParams() )
        if receiver ~= nil then
            this.PrintTableEntries("receiverParamSetsBase: ", ZetaEquipParameters.equipParameters.receiverParamSetsBase[receiver[3]+1])
            this.PrintTableEntries("receiverParamSetsWobbling: ", ZetaEquipParameters.equipParameters.receiverParamSetsWobbling[receiver[4]+1])
            this.PrintTableEntries("receiverParamSetsSystem: ", ZetaEquipParameters.equipParameters.receiverParamSetsSystem[receiver[5]+1])
            this.PrintTableEntries("receiverParamSetsSound: ", ZetaEquipParameters.equipParameters.receiverParamSetsSound[receiver[6]+1])
        end
    end
end

function this.PrintTableEntries(modLog, entries)
    if entries ~= nil and next(entries)then
        local string = "{"
        for i,value in ipairs(entries) do --Tables returned by all mods
            string = string.."\""..value.."\","
        end
        string = string .."},"
        InfCore.Log(modLog..string,false,true)
    end
end

return this